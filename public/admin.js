...
async function saveEconomySetting(key) {
  ...
  } catch (error) { setEconomyState(error.message || 'Unable to save economy setting.', true); }
...
function walletCard(wallet) {
  const source = wallet.currency === 'DZP' ? `<small>Earned ${wallet.earned_dzp} · Converted ${wallet.converted_dzp} · Purchased ${wallet.purchased_dzp}</small>` : '<small>Canonical wallet balance</small>';
...
function renderUserProfile(data) {
  ...
  const syncSource = () => { source.disabled = currency.value !== 'DZP'; };
  currency.addEventListener('change', syncSource); syncSource();
...
async function loadUsers() {
  ...
  renderUserList(data.users || []); setUsersState(`${(data.users || []).length} member(s) found.`);
}
async function loadUserProfile(userId) {
  selectedAdminUserId = userId; setUsersState('Loading profile…');
  try {
    const response = await fetch(`/api/admin/users/${encodeURIComponent(userId)}`, { headers: { 'X-Telegram-Init-Data': telegram.initData }, cache: 'no-store' });
    const data = await response.json();
    if (!response.ok) throw new Error(data.error || 'Unable to load member profile');
    renderUserProfile(data); setUsersState('Profile loaded.');
...
async function submitBalanceAdjustment(event) {
  ...
  try {
    const response = await fetch(`/api/admin/users/${encodeURIComponent(selectedAdminUserId)}/balance-adjustment`, { method: 'POST', headers: { 'Content-Type': 'application/json', 'X-Telegram-Init-Data': telegram.initData }, body: JSON.stringify(payload) });
    const data = await response.json();
    if (!response.ok) throw new Error(data.error || 'Unable to apply balance adjustment');
    setUsersState(data.duplicate ? 'Existing adjustment returned (idempotent).' : 'Adjustment applied and recorded in ledger.');
    await loadUserProfile(selectedAdminUserId);
  } catch (error) { setUsersState(error.message || 'Unable to apply balance adjustment.', true); }
