...
function taskAdminHeaders(json = false) {
  return {
    ...
    "X-Telegram-Init-Data": taskAdminTelegram?.initData || "",
  };
}
...
async function loadAdminTasks() {
  try {
    ...
    const response = await fetch("/api/admin/tasks?status=pending_review", {
    ...
    setTaskAdminState(
      `${(data.tasks || []).length} campaign(s) awaiting review.`,
    );
  } catch (error) {
    setTaskAdminState(error.message || "Unable to load campaigns.", true);
  }
}
async function reviewTask(taskId, action) {
  ...
  try {
    ...
    setTaskAdminState(
      data.duplicate
        ? "Existing review returned (idempotent)."
        : "Review applied and audited.",
    );
    await loadAdminTasks();
  } catch (error) {
    setTaskAdminState(error.message || "Review failed.", true);
  }
}
document
  .getElementById("adminTaskCampaignRefresh")
  .addEventListener("click", loadAdminTasks);
