...
async function copyCanonicalReferralLink() {
  ...
  if (!initData)
    throw new Error('Open DzMoney inside Telegram to load your referral link.');
  const response = await fetch('/api/me', {
    headers: { 'X-Telegram-Init-Data': initData },
  });
  const data = await response.json();
  if (!response.ok || !data.user?.referralLink)
    throw new Error(data.error || 'Referral link is unavailable.');
