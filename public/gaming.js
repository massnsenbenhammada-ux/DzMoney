(() => {
  ...
  function formatGamingAdFailure(providerId, stage, error) {
    ...
    if (stage === "start")
      return `${providerName}: could not start the ad session${detail}`;
    if (stage === "ready")
      return `${providerName}: the ad SDK is not ready${detail}`;
    if (stage === "show")
      return `${providerName}: the advertisement could not be displayed${detail}`;
    if (stage === "complete")
      return `${providerName}: the advertisement was shown, but completion could not be confirmed${detail}`;
  ...
  function ensureSpinWheel() {
    ...
    const card =
      root.querySelector("[data-spin-wheel-host]") ||
      root.querySelector("[data-spin-result]")?.parentElement;
  ...
  function renderRewardLists() {
    const spinList = root.querySelector(
      '[data-gaming-view="spin"] .gaming-reward-list',
    );
    if (spinList)
      spinList.innerHTML = wheelResults
        .map(
          (result) =>
            `<span class="gaming-pill gaming-pill-${result}">${wheelLabels[result]}</span>`,
        )
        .join("");
    const diggingList = root.querySelector(
      '[data-gaming-view="digging"] .gaming-reward-list',
    );
    if (diggingList)
      diggingList.innerHTML = wheelResults
        .map(
          (result) =>
            `<span class="gaming-pill gaming-pill-${result === "extra_spin" ? "extra_axe" : result}">${result === "extra_spin" ? "+1 AXE" : wheelLabels[result]}</span>`,
        )
        .join("");
  }
  ...
  function render() {
    ...
    setAll(
      "[data-energy]",
      `${account.energy_remaining ?? 0}/${gaming.config?.digging?.energy ?? 3}`,
    );
    setAll(
      "[data-spin-ad-count]",
      `${gaming.adCounts?.spin || 0}/${dailyAdLimit}`,
    );
    setAll(
      "[data-dig-ad-count]",
      `${gaming.adCounts?.digging || 0}/${dailyAdLimit}`,
    );
    root.querySelectorAll("[data-spin-ad-bar]").forEach((bar) => {
      bar.style.width = `${Math.min(100, ((gaming.adCounts?.spin || 0) / dailyAdLimit) * 100)}%`;
    });
    root.querySelectorAll("[data-dig-ad-bar]").forEach((bar) => {
      bar.style.width = `${Math.min(100, ((gaming.adCounts?.digging || 0) / dailyAdLimit) * 100)}%`;
    });
    root
      .querySelectorAll("[data-gaming-view]")
      .forEach((el) =>
        el.classList.toggle(
          "gaming-hidden",
          el.dataset.gamingView !== state.view,
        ),
      );
    root.querySelectorAll("[data-dig-start]").forEach((btn) => {
      btn.disabled = state.busy || account.axes < 1 || !!gaming.activeSession;
    });
    root.querySelectorAll("[data-spin-action]").forEach((btn) => {
      btn.disabled = state.busy || account.spins < 1;
    });
    root.querySelectorAll("[data-gaming-ad]").forEach((btn) => {
      const reachedLimit =
        (gaming.adCounts?.[btn.dataset.gamingAd] || 0) >= dailyAdLimit;
      btn.disabled = state.busy || reachedLimit;
      if (!state.busy)
        btn.textContent = reachedLimit ? "DAILY LIMIT REACHED" : "WATCH AD";
    });
  ...
  function animateWheel(result) {
    ...
    if (reduced) {
      ...
      wheel
        .querySelector(`[data-spin-wheel-segment="${result}"]`)
        ?.classList.add("is-winning");
    ...
    return new Promise((resolve) => {
      window.setTimeout(() => {
        ...
        wheel.classList.add("is-winner");
        wheel
          .querySelectorAll("[data-spin-wheel-segment]")
          .forEach((segmentEl) => segmentEl.classList.remove("is-winning"));
        wheel
          .querySelector(`[data-spin-wheel-segment="${result}"]`)
          ?.classList.add("is-winning");
        ...
        window.setTimeout(
          () =>
            root
              .querySelector(".spin-wheel-pointer")
              ?.classList.remove("is-bouncing"),
          260,
        );
        resolve();
      }, 3200);
  ...
  async function spin() {
    ...
    try {
      const response = await api("/api/gaming/spin", {
        method: "POST",
        body: JSON.stringify({ idempotencyKey: idempotencyKey("gaming-spin") }),
      });
  ...
  async function startDigging() {
    ...
    render();
    try {
      await api("/api/gaming/digging/start", { method: "POST" });
      await load();
    } catch (error) {
      toast(error.message);
    } finally {
  ...
  async function reveal(tileId) {
    ...
    try {
      const session = state.gaming.activeSession;
      const response = await api("/api/gaming/digging/reveal", {
        method: "POST",
        body: JSON.stringify({ sessionId: session.id, tileId }),
      });
      root.querySelector("[data-dig-result]").textContent = formatReward(
        response.tile.reward,
  ...
  async function watchAd(game) {
    ...
    try {
      const response = await api("/api/gaming/ads/start", {
        ...
        body: JSON.stringify({
          game,
          idempotencyKey: idempotencyKey(`gaming-ad:${game}`),
        }),
      });
      ...
      await adapter.ready;
      stage = "show";
      const completion = await adapter.handler({
        requestVar: "gaming",
        adEventId: response.adEventId,
        ymid: response.externalAdId,
      });
      stage = "complete";
      if (button) button.textContent = "CREDITING…";
      await load();
      if (completion.rewarded === true) {
        if (typeof window.showRewardOutcome === "function")
          window.showRewardOutcome(completion);
        else
          toast(
            completion.duplicate
              ? "Ad already credited."
              : "Ad watched — reward credited.",
          );
      ...
      toast(
        `Gaming Ad failed — ${formatGamingAdFailure(providerId, stage, error)}`,
      );
    } finally {
      setBusy(false);
      if (button) {
  ...
  root.addEventListener("keydown", (event) => {
    if (
      ...
      event.target.closest("[data-spin-wheel]")
    ) {
