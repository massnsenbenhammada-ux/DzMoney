(() => {
  ...
  async function showPromoAd(result) {
    ...
    if (!provider?.handler)
      throw new Error("Promo advertisement provider is unavailable");
  ...
  async function waitForReward(redemptionId) {
    ...
    while (Date.now() < deadline) {
      const result = await window.DzMoneyPromoApi(
        `/api/promo/redemption/${redemptionId}`,
      );
      if (result.redemption?.status === "verified") return result.redemption;
      if (
        result.redemption?.status === "expired" ||
        result.redemption?.status === "rejected"
      )
        throw new Error("Promo claim could not be completed");
      await wait(2000);
    }
  ...
  async function redeem() {
    ...
    try {
      const result = await window.DzMoneyPromoApi("/api/promo/redeem", {
        method: "POST",
        body: JSON.stringify({
          code,
      ...
      if (result.status === "verified") {
        setStatus(
          `Reward credited: ${result.rewardAmount} ${result.rewardCurrency}`,
        );
      ...
      setStatus(
        `Reward credited: ${redemption.rewardAmount} ${redemption.rewardCurrency}`,
      );
      input.value = "";
      await window.DzMoneyPromoRefresh?.();
    } catch (error) {
