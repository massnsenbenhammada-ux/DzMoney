(() => {
  ...
  promo.innerHTML =
    '<span class="premium-promo-icon" aria-hidden="true">✦</span><div><strong>Earn smarter with DzMoney</strong><span>Tasks, Gaming and Squad activity stay verified by the server.</span></div>';
  ...
  const open = () => {
    const eyebrow =
      document.querySelector(".welcome-row .eyebrow")?.textContent ||
      "DZ MONEY USER";
    ...
    document.getElementById("premiumAvatar").textContent =
  ...
  document.addEventListener("click", (event) => {
    ...
    if (event.target === sheet || event.target.closest(".profile-close"))
      close();
    const nav = event.target.closest("[data-profile-go]");
    if (nav) {
      close();
      document.querySelector(`[data-go="${nav.dataset.profileGo}"]`)?.click();
    }
  });
...
(() => {
  ...
  const conversion =
    wallet &&
    [...wallet.querySelectorAll(".info-card")].find(
      (item) =>
        item.querySelector("strong")?.textContent.trim() === "Conversion",
    );
  if (
    conversion &&
    !conversion.classList.contains("phase11-conversion-notice")
  ) {
  ...
  const updateSquadPresentation = async () => {
    ...
    if (homeSummary)
      homeSummary.textContent = squad
        ? `Squad #${squad.id} • Level ${squad.tierLevel ?? "—"} • ${squad.memberCount} Members`
        : "Squad #— • Level — • — Members";
    ...
    if (level)
      level.textContent =
        squad?.tierLevel != null ? `Level ${squad.tierLevel}` : "—";
    if (members) members.textContent = squad ? String(squad.memberCount) : "—";
    if (required)
      required.textContent =
        squad?.requiredMembers != null ? String(squad.requiredMembers) : "—";
    if (progress)
      progress.textContent =
        squad?.progressPercent != null ? `${squad.progressPercent}%` : "—";
  };

  const updateGaming = () => {
    const spin =
      document.querySelector("[data-spin-balance]")?.textContent || "0";
    const axe =
      document.querySelector("[data-axe-balance]")?.textContent || "0";
    document
      .querySelector("[data-home-spin-balance]")
      ?.replaceChildren(document.createTextNode(spin));
    document
      .querySelector("[data-home-axe-balance]")
      ?.replaceChildren(document.createTextNode(axe));
    document
      .querySelector("[data-phase11-drawer-spins]")
      ?.replaceChildren(document.createTextNode(spin));
    document
      .querySelector("[data-phase11-drawer-axes]")
      ?.replaceChildren(document.createTextNode(axe));
  };
  ...
  const loadHomeStatus = async () => {
    ...
    try {
      ...
      if (data.status === "cooldown") {
        ...
        detail.textContent =
          "Your next check-in will unlock after the cooldown.";
      } else if (data.status === "pending") {
        title.textContent = "Daily check-in verifying";
        detail.textContent = "Verification is still pending on the server.";
      } else {
  ...
  const openDrawer = async (event) => {
    ...
    document.getElementById("phase11DrawerUsername").textContent = user.username
      ? `@${user.username}`
      : "Telegram account";
    document.getElementById("phase11DrawerAvatar").textContent =
      String(name).charAt(0).toUpperCase() || "D";
    document.getElementById("phase11DrawerCoin").textContent =
      document.getElementById("coinBalance")?.textContent || "0";
    document.getElementById("phase11DrawerDzx").textContent =
      document.getElementById("dzxBalance")?.textContent || "0";
    document.getElementById("phase11DrawerDzp").textContent =
      document.getElementById("dzpBalance")?.textContent || "0";
    const photo = document.getElementById("phase11DrawerPhoto");
    if (user.photoUrl) {
      photo.src = user.photoUrl;
      photo.hidden = false;
      document.getElementById("phase11DrawerAvatar").hidden = true;
    } else {
      photo.hidden = true;
      document.getElementById("phase11DrawerAvatar").hidden = false;
    }
    drawer.hidden = false;
    requestAnimationFrame(() => drawer.classList.add("open"));
    try {
      const [squad, daily] = await Promise.all([
        api("/api/squad"),
        api("/api/daily-checkin/status"),
      ]);
      document.getElementById("phase11DrawerSquad").textContent = squad.squad
        ? squad.squad.membershipStatus || "Active"
        : "No Squad";
      document.getElementById("phase11DrawerMembers").textContent = squad.squad
        ? String(squad.squad.memberCount)
        : "0";
      document.getElementById("phase11DrawerLevel").textContent =
        squad.squad?.tierLevel != null ? String(squad.squad.tierLevel) : "—";
      document.getElementById("phase11DrawerDaily").textContent =
        daily.status === "cooldown"
          ? "Cooldown"
          : daily.status === "pending"
            ? "Verifying"
            : "Ready";
      document.getElementById("phase11DrawerDailyDetail").textContent =
        daily.status === "cooldown"
          ? "Next check-in is locked by the server cooldown."
          : "Daily check-in is available through the verified flow.";
    } catch {}
    updateGaming();
  };
  const closeDrawer = (event) => {
    if (event && !event.target.closest("[data-phase11-drawer-close]")) return;
  ...
  document.addEventListener("keydown", (event) => {
    if (event.key === "Escape" && drawer.classList.contains("open"))
      closeDrawer({ target: drawer.querySelector(".phase11-drawer-overlay") });
  });
