(() => {
  ...
  async function runAdsgram(button, result) {
    ...
    if (!shown || shown.done !== true)
      throw new Error("AdsGram reward was not completed");
    const completion = await api(
      "/api/daily-tasks/advertisement/client-complete",
      { method: "POST", body: JSON.stringify({ adEventId: result.adEventId }) },
    );
    if (completion.progress)
      window.__DzMoneyDailyAdProgress = completion.progress;
    return waitForReward(result.adEventId);
  }
  async function run(button) {
    ...
    try {
      ...
      if (finalized.progress)
        window.__DzMoneyDailyAdProgress = finalized.progress;
      if (typeof window.loadMe === "function") await window.loadMe();
      if (typeof window.loadDailyAdProgress === "function")
        await window.loadDailyAdProgress();
      if (typeof window.showRewardOutcome === "function")
        window.showRewardOutcome(finalized, null);
    } catch (error) {
  ...
  document.addEventListener(
    ...
    (event) => {
      const button = event.target.closest(
        '.daily-system-action[data-system-key="view_ads"]',
      );
      if (!button) return;
