...
function renderDailyState(state) {
  ...
  const reason = state.activationReason
    ? `<span> · ${escapeHtml(state.activationReason)}</span>`
    : '';
  ...
  try {
    ...
    container.innerHTML = invitations.length
      ? invitations
          .map(
            (item) =>
              `<article class="squad-invitation"><div><strong>Squad #${escapeHtml(item.squadId)}</strong><small>Pending invitation</small></div><button class="squad-btn primary" type="button" data-accept-invitation="${escapeHtml(item.id)}">Accept</button></article>`,
          )
          .join('')
      : '<div class="squad-empty-inline">No pending invitations.</div>';
  } catch (error) {
...
async function markAdsgramSquadAdStarted(adEventId) {
  return api('/api/daily-tasks/advertisement/client-started', {
    method: 'POST',
...
async function completeAdsgramSquadAd(adEventId) {
  return api('/api/daily-tasks/advertisement/client-complete', {
    method: 'POST',
    body: JSON.stringify({ adEventId }),
  });
}
...
async function watchSquadAd() {
  ...
  try {
    if (typeof window.DzMoneyAdClient?.getProvider !== 'function')
      throw new Error('Advertisement providers are unavailable');
    const squadAds = await loadSquadAds();
    const taskId = Number(squadAds?.task?.id);
    if (!Number.isInteger(taskId) || taskId <= 0)
      throw new Error('Squad Ads task is unavailable');
    const response = await api('/api/tasks/advertisement/start', {
      method: 'POST',
      body: JSON.stringify({ taskId, idempotencyKey: idempotencyKey() }),
    });
    const adapter = window.DzMoneyAdClient.getProvider(response.providerId);
    if (!adapter?.handler || typeof adapter.handler !== 'function')
      throw new Error('Advertisement provider is unavailable');
    button.textContent = 'WATCHING…';
    await adapter.ready;
    const onStart =
      response.providerId === 'adsgram'
        ? () => markAdsgramSquadAdStarted(response.adEventId)
        : undefined;
    ...
    if (response.providerId === 'adsgram')
      await completeAdsgramSquadAd(response.adEventId);
    button.textContent = 'VERIFYING…';
    const deadline = Date.now() + 30000;
    while (Date.now() < deadline) {
      const state = await api(
        `/api/squad/ads?adEventId=${encodeURIComponent(response.adEventId)}`,
      );
    ...
    if (status)
      status.textContent = String(error.message || 'Advertisement unavailable');
  }
}
...
async function inviteUser(event) {
  ...
  try {
    ...
    await api('/api/squad/invitations', {
      method: 'POST',
      body: JSON.stringify({
        squadId: squad.squad.id,
        inviteeTelegramUserId: input.value.trim(),
      }),
    });
    input.value = '';
...
async function purchaseMembership(maxMembers) {
  const button = document.querySelector(
    `[data-squad-tier="${String(maxMembers)}"]`,
  );
  if (button) {
    button.disabled = true;
...
async function acceptInvitation(event) {
  ...
  try {
    await api(
      `/api/squad/invitations/${button.dataset.acceptInvitation}/accept`,
      { method: 'POST' },
    );
    await loadSquad();
  } catch (error) {
...
async function loadSquad() {
  ...
  try {
    const [data, tiers, stateData, ads] = await Promise.all([
      api('/api/squad'),
      loadPaidTiers(),
      api('/api/squad/daily-state'),
      loadSquadAds(),
    ]);
...
document.addEventListener('submit', (event) => {
  if (event.target.id === 'squadInviteForm')
    inviteUser(event).catch((error) =>
      alert(String(error.message || 'Invitation failed')),
    );
});
