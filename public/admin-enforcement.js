...
async function loadEnforcementState() {
  try {
    ...
    const response = await fetch(`/api/admin/users/enforcement/${encodeURIComponent(userId)}`, {
      headers: authHeaders(),
      cache: 'no-store',
    });
    ...
    setEnforcementState(error.message || 'Unable to load account status.', true);
  }
}
async function applyEnforcement(action) {
  try {
    ...
    enforcementMembership.textContent = `Membership: ${data.membershipStatus || 'none'}`;
    setEnforcementState(
      data.duplicate ? 'Existing action returned (idempotent).' : 'Action applied and audited.',
    );
  } catch (error) {
    setEnforcementState(error.message || 'Unable to update account status.', true);
  } finally {
