...
function creatorIdempotencyKey() {
  if (!creatorTaskState.idempotencyKey)
    creatorTaskState.idempotencyKey = `creator-task:${crypto.randomUUID()}`;
...
const VERIFICATION_DESCRIPTIONS = Object.freeze({
  ...
  url_format_match:
    'The external Mini App referral URL must match the campaign URL format.',
  bot_api:
    'The Telegram target is verified server-side through the Bot API contract.',
});
function verificationOptions() {
  ...
  return methods.map((method) => [
    ...
    method === 'click_proof'
      ? 'Click Proof'
      : method === 'url_format_match'
        ? 'URL Format Match'
        : method === 'bot_api'
          ? 'Bot API'
          : method,
    VERIFICATION_DESCRIPTIONS[method] || 'Server-defined verification method.',
  ]);
}
function providerForSocial() {
  return (
    (creatorTaskState.contract?.providerContracts || []).find(
      (item) => item.method === creatorTaskState.verification,
    ) || null
  );
}
...
function categoryButtons(active) {
  return `<div class="creator-category-grid">${[
    ['game', 'Game'],
    ['social', 'Social'],
    ['web', 'Web'],
    ['special', 'Special'],
    ...
    .map(
      ([key, label]) =>
        `<button type="button" class="creator-category ${active === key ? 'active' : ''}" data-creator-category="${key}">${label}</button>`,
    )
    .join('')}</div>`;
}
function renderCreatorForm() {
  ...
  const target = Math.max(
    minTarget,
    Number(creatorEl('creatorTarget')?.value || minTarget),
  );
  const options = verificationOptions();
  if (!options.length) {
    form.innerHTML =
      '<article class="info-card"><strong>No verification methods are configured for this task type.</strong></article>';
    return;
  }
  if (!options.some(([key]) => key === creatorTaskState.verification))
    creatorTaskState.verification = options[0][0];
  const provider =
    creatorTaskState.verification === 'bot_api' ? providerForSocial() : null;
  ...
  bindCreatorCategories();
  creatorEl('creatorTarget')?.addEventListener(
    'input',
    updateCreatorCampaignCost,
  );
  document.querySelectorAll('[data-creator-verification]').forEach((button) =>
    button.addEventListener('click', () => {
      creatorTaskState.verification = button.dataset.creatorVerification;
      renderCreatorForm();
    }),
  );
}
function bindCreatorCategories() {
  document
    .querySelectorAll('[data-creator-category]')
    .forEach((button) =>
      button.addEventListener('click', () =>
        setCreatorTaskType(button.dataset.creatorCategory),
      ),
    );
}
function updateCreatorCampaignCost() {
  ...
  const value = document.querySelector(
    '.creator-price-total .creator-price-value',
  );
  if (value)
    value.textContent =
      target > 0 && price > 0 ? `${formatDZX(target * price)} DZX` : '—';
}
async function setCreatorTaskType(taskType) {
  ...
  if (taskType === 'special') {
    creatorTaskState.contract = null;
    renderCreatorForm();
    return;
  }
...
function creatorConfig() {
  ...
  if (creatorTaskState.verification === 'bot_api') {
    const channel =
      creatorEl('creatorProviderField_channel')?.value?.trim() || '';
...
async function loadCreatorContract() {
  ...
  if (!form) return;
  form.innerHTML =
    '<article class="info-card"><strong>Loading campaign contract…</strong></article>';
  try {
    creatorTaskState.contract = await creatorApi(
      `/api/creator/tasks/contracts/${encodeURIComponent(creatorTaskState.taskType)}`,
    );
    renderCreatorForm();
  } catch (error) {
    form.innerHTML = `<article class="info-card"><strong>Unable to load contract</strong><p>${String(error.message || 'Please try again.')}</p></article>`;
  }
}
async function createCreatorTask(event) {
  event.preventDefault();
  if (creatorTaskState.taskType === 'special' || creatorTaskState.createdTaskId)
    return;
  ...
  try {
    const result = await creatorApi('/api/creator/tasks', {
      method: 'POST',
      body: JSON.stringify({
        taskType: creatorTaskState.taskType,
    ...
    creatorToast(
      `Task created. Reserved ${formatDZX(result.campaign?.campaignCostDZX)} DZX.`,
    );
    button.textContent = 'Task created';
    const review = creatorEl('creatorReviewSubmit');
    if (review) review.disabled = !creatorTaskState.createdTaskId;
  } catch (error) {
...
async function submitCreatorTaskForReview(event) {
  ...
  try {
    const result = await creatorApi(
      `/api/creator/tasks/${encodeURIComponent(creatorTaskState.createdTaskId)}/submit`,
      { method: 'POST', body: '{}' },
    );
    creatorToast(
      result.task?.status === 'pending_review'
...
function setCreatorPanelVisible(visible) {
  ...
  document
    .querySelectorAll('[data-task-mode]')
    .forEach((tab) =>
      tab.classList.toggle(
        'active',
        tab.dataset.taskMode === (visible ? 'creator' : 'tasks'),
      ),
    );
...
document.addEventListener('click', (event) => {
  ...
  if (tab) {
    setCreatorPanelVisible(tab.dataset.taskMode === 'creator');
    return;
  }
  if (event.target.closest('#creatorReviewSubmit'))
    submitCreatorTaskForReview(event);
});
