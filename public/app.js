...
async function api(
  ...
  // Telegram initData is sent only in the dedicated header. Do not send
  // client-provided Telegram user objects or initData in query strings/body
  // fields where they can leak into logs or be mistaken for identity.
...
async function startTask(taskId) {
  ...
  try {
    const data = await api(`/api/tasks/${encodeURIComponent(taskId)}/start`, {
      method: "POST"
    });
    ...
    if (remaining <= 0) {
      await claimTask(taskId, button);
      return;
    }

    if (button) button.textContent = formatTime(remaining);
...
async function withdraw() {
  ...
  const amountBux = Math.floor(bux);
