...
test("Game Click Proof uses the canonical campaign URL without a provider contract", () => {
  const config = {
    campaignUrl: "https://example.com/game",
    verification: { method: "click_proof" },
  };
  assert.doesNotThrow(() =>
    validateCreatorProviderConfiguration("game", config),
  );
...
test("Game URL Format Match verifier uses campaign URL format and not the exact referral value", async () => {
  ...
  const verifier = resolveTrustedTaskVerifier({
    config,
    userSubmittedUrl: "https://t.me/MBuxBot/app?startapp=r_8654896543",
  });
  assert.equal(await verifier({ attemptId: 1 }), true);
  const mismatch = resolveTrustedTaskVerifier({
    config,
    userSubmittedUrl: "https://t.me/surf_earn_bot/app?startapp=r_5459324721",
  });
...
test("Social Bot API uses the existing Telegram provider contract", () => {
  ...
  assert.doesNotThrow(() =>
    validateCreatorProviderConfiguration("social", config),
  );
  ...
  assert.deepEqual(resolved.verification.requirements, {
...
test("Partner verification remains provider-defined without becoming a Creator method", () => {
  ...
  assert.throws(
    () => validateCreatorProviderConfiguration("special", resolved),
