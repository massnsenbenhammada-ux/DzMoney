...
const { parseTestAll, validateTestAll } = require("../scripts/test-governance");
...
test("test governance parses npm run entries and direct node test files", () => {
  ...
  assert.deepEqual(entries, [
    ...
    {
      type: "file",
      path: "./scripts/test-alpha.js",
      command: "node ./scripts/test-alpha.js",
    },
    {
      type: "file",
      path: "./tests/alpha.test.js",
      command: "node --test ./tests/alpha.test.js",
    },
  ]);
});
  ...
  const result = validateTestAll(
    {
      scripts: {
        "test:all":
          "npm run test:alpha && npm run test:alpha && npm run test:all",
