...
test("Game URL format rejects a changed path or query key", () => {
  ...
  assert.equal(
    matchesUrlFormat(reference, "https://t.me/MBuxBot?startapp=r_8654896543"),
    false,
  );
  assert.equal(
    matchesUrlFormat(reference, "https://t.me/MBuxBot/app?start=r_8654896543"),
    false,
  );
});
...
test("Game URL format rejects malformed or extra-query URLs", () => {
  ...
  assert.equal(
    matchesUrlFormat(
      reference,
      "https://t.me/MBuxBot/app?startapp=r_8654896543&x=1",
    ),
    false,
  );
});
