...
test("rejects cross-network TON addresses", () => {
  assert.throws(
    () => assertAddressNetwork(MAINNET, "testnet"),
    /does not match testnet/,
  );
  assert.throws(
    () => assertAddressNetwork(TESTNET, "mainnet"),
...
test("rejects malformed TON addresses", () => {
  assert.throws(
    () => normalizeTonAddress("UQA-invalid"),
    /Invalid TON user-friendly address/,
  );
  assert.throws(
    () => normalizeTonAddress("EQ123"),
    /Invalid TON user-friendly address/,
  );
});
...
test("rejects a valid-length address with an invalid checksum", () => {
  ...
  assert.throws(
    () => normalizeTonAddress(tampered),
    /Invalid TON address checksum/,
  );
});
