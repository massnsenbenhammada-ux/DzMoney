...
test('Phase 10 migration defines canonical promo campaigns and redemptions', () => {
  const migration = fs.readFileSync(path.join(root, 'migrations/047_promo_codes.sql'), 'utf8');
...
test('Promo service exposes redeem and finalization boundaries', () => {
  ...
  assert.throws(() => service.normalizePromoCode('bad code'), /Invalid promo code/);
});

test('Promo reward validation only permits COIN or DZX', () => {
  const { normalizeReward } = require('../src/services/promo-code-service');
  assert.deepEqual(normalizeReward({ rewardCurrency: 'coin', rewardAmount: '1000' }), { currency: 'COIN', amount: '1000' });
  assert.deepEqual(normalizeReward({ rewardCurrency: 'DZX', rewardAmount: '2.5' }), { currency: 'DZX', amount: '2.5' });
  assert.throws(() => normalizeReward({ rewardCurrency: 'DZP', rewardAmount: 1 }), /reward currency/);
  assert.throws(() => normalizeReward({ rewardCurrency: 'COIN', rewardAmount: 0 }), /positive/);
});

test('Promo routes are mounted and protected by Telegram/admin authentication boundaries', () => {
  const server = fs.readFileSync(path.join(root, 'server.js'), 'utf8');
  const routes = fs.readFileSync(path.join(root, 'src/http/promo-code-routes.js'), 'utf8');
  const adminRoutes = fs.readFileSync(path.join(root, 'src/http/admin-promo-code-routes.js'), 'utf8');
...
test('Promo advertisement is an explicit provider context and never a task verification event', () => {
  ...
  const event = fs.readFileSync(path.join(root, 'src/services/ad-event-service.js'), 'utf8');
