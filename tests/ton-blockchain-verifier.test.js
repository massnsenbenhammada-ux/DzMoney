...
function transaction({
  ...
  return {
    ...
    mc_block_seqno: 123,
    description: { aborted },
    in_msg: { destination, value, bounced },
  };
}
function masterchain(seqno = 130) {
  return { last: { seqno } };
}
function finalizedFetch({
  currentSeqno = 130,
  traceOverrides = {},
  tx = transaction(),
} = {}) {
  return async (url) => {
    ...
    if (url.includes("/traces?"))
      return response({ traces: [trace(TX_HASH, traceOverrides)] });
...
test("requires a strict transaction hash", () => {
  ...
  assert.throws(
    () => assertTransactionHash("short"),
    /64-character hexadecimal/,
  );
  assert.throws(
    () => assertTransactionHash("z".repeat(64)),
    /64-character hexadecimal/,
  );
});
  ...
  const fetchImpl = async (url) =>
    url.includes("/transactions?")
      ...
      : url.includes("/traces?")
        ? response({
            traces: [
              trace(TX_HASH, { mc_seqno_end: 123, is_incomplete: true }),
            ],
          })
        : response(masterchain());
  const result = await verifyTonDeposit({
...
test("holds when finality trace is incomplete", async () => {
  const fetchImpl = async (url) =>
    url.includes("/transactions?")
      ...
      : url.includes("/traces?")
        ? response({
            traces: [trace(TX_HASH, { mc_seqno_end: 0, is_incomplete: true })],
          })
        : response(masterchain());
  const result = await verifyTonDeposit({
    network: "mainnet",
