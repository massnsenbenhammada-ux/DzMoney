...
test('Daily View Ads runs through the real UI and canonical HTTP reward path', async ({ page, request, baseURL }) => {
  ...
  const cleanup = async () => {
    const me = await request.get(new URL('/api/me', baseURL).toString(), { headers: { 'X-Telegram-Init-Data': initData } }).catch(() => null);
    if (!me?.ok()) return;
