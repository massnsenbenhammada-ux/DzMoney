...
function buildInitData(userId) {
  ...
  params.set('user', JSON.stringify({ id: userId, first_name: 'Phase 14 E2E' }));
...
test('Daily View Ads runs through the real UI and canonical HTTP reward path', async ({ page, request }) => {
  test.skip(!process.env.BOT_TOKEN || !process.env.MONETAG_POSTBACK_SECRET, 'Phase 14 E2E requires repository provider/auth secrets');
  ...
  try {
    ...
    const postback = new URL('/api/ads/monetag/postback', 'http://127.0.0.1:3000');
    ...
    const postbackResponse = await request.get(postback.pathname + postback.search);
