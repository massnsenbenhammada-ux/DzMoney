...
function buildInitData(userId) {
  ...
  params.set(
    'user',
    JSON.stringify({ id: userId, first_name: 'DzMoney Real Monetag' }),
  );
...
test('Daily View Ads credits 20 consecutive real Monetag ads', async ({
  page,
  request,
}) => {
  test.skip(
    process.env.REAL_MONETAG_E2E !== '1',
    'Explicit opt-in: REAL_MONETAG_E2E=1',
  );

  const baseUrl =
    process.env.REAL_MONETAG_BASE_URL ||
  ...
  page.on('request', (requestEvent) => {
    if (requestEvent.url().includes('libtl.com/sdk.js'))
      seenMonetagRequests.push(requestEvent.url());
  });
  page.on('response', async (response) => {
    ...
    if (
      response.url().endsWith('/api/daily-tasks/advertisement/finalize') &&
      response.request().method() === 'POST'
    ) {
      try {
        const body = await response.json();
  ...
  try {
    ...
    await expect
      .poll(async () => page.evaluate(() => typeof window.show_11627577))
      .toBe('function', { timeout: 20000 });
    ...
    for (let expected = 1; expected <= 20; expected += 1) {
      ...
      await expect
        .poll(() => readDailyViewText(page), { timeout: 75000 })
        .toContain(`${expected}/20 watched`);
      await expect
        .poll(
          () =>
            finalizedEvents.filter(
              (item) => item.progress?.completed === expected,
            ).length,
          { timeout: 10000 },
        )
        .toBe(1);
      const rewardResponse = finalizedEvents.find(
        (item) => item.progress?.completed === expected,
      );
      ...
      if (expected < 20) {
        await expect(page.locator('[data-task-action="view_ads"]')).toBeEnabled(
          { timeout: 10000 },
        );
      }
    }
    ...
    expect(
      finalizedEvents.every((item) => item.rewarded === true),
    ).toBeTruthy();
    expect(
      finalizedEvents.every(
        (item) =>
          item.reward &&
          item.reward.coin === 1000 &&
          item.reward.dzx === 1 &&
          item.reward.dzp === 1,
      ),
    ).toBeTruthy();
    ...
    await expect
      .poll(() => readDailyViewText(page), { timeout: 10000 })
      .toContain('20/20 watched');
  } finally {
    await cleanup();
  }
});
