...
async function ensureWithdrawalSchema() {
  ...
  const required = await pool.query(
    \`SELECT column_name FROM information_schema.columns WHERE table_schema='public' AND table_name='withdrawals' AND column_name = ANY($1::text[])\`,
  ...
  await pool.query(\`UPDATE withdrawals SET fee_bux=COALESCE(fee_bux,0), net_bux=CASE WHEN net_bux IS NULL OR net_bux=0 THEN amount_bux-COALESCE(fee_bux,0) ELSE net_bux END, amount_ton=COALESCE(amount_ton,0), destination=COALESCE(destination,''), status=COALESCE(status,'pending'), admin_note=COALESCE(admin_note,''), created_at=COALESCE(created_at,0), updated_at=COALESCE(updated_at,created_at,0)\`);

  for (const [key, value] of Object.entries({ minimum_withdraw_bux:'2000', withdrawal_fee_bux:'0' })) {
    await pool.query(\`INSERT INTO settings (key,value,updated_at) VALUES ($1,$2,$3) ON CONFLICT (key) DO NOTHING\`, [key,value,Date.now()]);
  }
}
...
`;
  ...
  const dailyFunction=`function bindDailyButton(){
  ...
  const dailyStart=source.indexOf('const dailyButton =');
  const dailyEnd=source.indexOf('// ============================\n// Navigation',dailyStart);
  if(dailyStart>=0&&dailyEnd>dailyStart){
    source=source.slice(0,dailyStart)+dailyFunction+'\n\n'+source.slice(dailyEnd);
  }else if(!source.includes('function bindDailyButton()')){
    source=replaceOnce(source,'// ============================\n// Navigation',dailyFunction+'\n\n// ============================\n// Navigation','Daily reward binding');
  }

  source=source.replace('updateDaily();\n\nloadUser();\ninitTonConnect();','updateDaily();\nbindDailyButton();\nloadUser();\ninitTonConnect();');
