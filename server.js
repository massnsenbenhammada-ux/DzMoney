...
async function initDatabase() {
  ...
  // In particular, older DzMoney databases may have a withdrawals table
  // without the user_id and/or amount_bux columns used by the current
  // withdrawal endpoint.  Adding those columns here prevents the request
  // from failing with PostgreSQL "column does not exist" errors.
  //
  // IMPORTANT: user_id is intentionally nullable and amount_bux gets a safe
  // zero default when repairing an old table. We cannot invent historical
  // user IDs or withdrawal amounts for legacy rows. New withdrawal requests
  // always provide both values explicitly.
  await pool.query(`
    ALTER TABLE withdrawals ADD COLUMN IF NOT EXISTS user_id TEXT;
    ALTER TABLE withdrawals ADD COLUMN IF NOT EXISTS amount_bux BIGINT DEFAULT 0;
  ...
  // Normalize legacy withdrawal rows after adding/repairing the columns.
  // Only calculate net_bux from a real amount; legacy rows repaired with the
  // zero default remain harmless until a new withdrawal is created.
  await pool.query(`
    UPDATE withdrawals
    SET
      amount_bux = COALESCE(amount_bux, 0),
      fee_bux = COALESCE(fee_bux, 0),
      net_bux = CASE
        WHEN amount_bux IS NULL THEN 0
        ELSE GREATEST(0, amount_bux - COALESCE(fee_bux, 0))
      END
    WHERE amount_bux IS NULL
       OR fee_bux IS NULL
       OR net_bux IS NULL
       OR net_bux = 0;
  `);
