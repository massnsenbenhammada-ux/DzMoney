...
async function initDatabase() {
  ...
  await pool.query(`
    ALTER TABLE withdrawals ADD COLUMN IF NOT EXISTS amount_ton NUMERIC(20,9) NOT NULL DEFAULT 0;
...
app.post("/api/withdrawals", requireSystemEnabled, requireTelegramAuth, async (req, res) => {
  ...
  try {
    ...
    const buxPerTon = Math.max(1, Math.floor(Number(await getSettingValue("bux_per_ton", "10000")) || 10000));
    ...
    // Store the gross requested TON amount in the legacy amount_ton column.
    const amountTon = (amount / buxPerTon).toFixed(9);
    const now = Date.now();
    ...
    res.json({
      ...
      withdrawal: {
        ...
        amountBux: amount,
        amountTon: Number(created.rows[0].amount_ton),
        feeBux: fee,
        netBux: net,
        destination,
        status: "pending",
