...
async function initDatabase() {
  ...
  await pool.query(`
    ALTER TABLE task_claims ADD COLUMN IF NOT EXISTS started_at BIGINT NOT NULL DEFAULT 0;
    ALTER TABLE task_claims ADD COLUMN IF NOT EXISTS reward BIGINT NOT NULL DEFAULT 0;
  `);

  // Protect new writes without blocking startup on legacy rows that may already
  // violate the invariant. Existing bad rows can be reviewed separately.
  await pool.query(`
    DO $$
    BEGIN
      IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'users_bux_nonnegative') THEN
        ALTER TABLE users ADD CONSTRAINT users_bux_nonnegative CHECK (bux >= 0) NOT VALID;
      END IF;
      IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'users_coins_nonnegative') THEN
        ALTER TABLE users ADD CONSTRAINT users_coins_nonnegative CHECK (coins >= 0) NOT VALID;
      END IF;
      IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'tasks_reward_nonnegative') THEN
        ALTER TABLE tasks ADD CONSTRAINT tasks_reward_nonnegative CHECK (reward >= 0) NOT VALID;
      END IF;
      IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'tasks_duration_nonnegative') THEN
        ALTER TABLE tasks ADD CONSTRAINT tasks_duration_nonnegative CHECK (duration >= 0) NOT VALID;
      END IF;
      IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'tasks_cooldown_nonnegative') THEN
        ALTER TABLE tasks ADD CONSTRAINT tasks_cooldown_nonnegative CHECK (cooldown_seconds >= 0) NOT VALID;
      END IF;
    END $$;
  `);
  ...
  const settings = {
    ...
    daily_reward_bux: "1",
    coins_per_bux: String(ECONOMY.COINS_PER_BUX),
    bux_per_ton: String(ECONOMY.BUX_PER_TON),
    ...
    for (const [taskId, claim] of Object.entries(claims || {})) {
      await pool.query(
        `INSERT INTO task_claims
         (user_id,task_id,claimed_at,started_at,reward)
         VALUES ($1,$2,$3,0,$4)
...
function formatUser(user) {
  return {
    ...
    bux: Number(user.bux),
    ton: Number(user.bux) / ECONOMY.BUX_PER_TON,
...
async function requireAdmin(req, res, next) {
  ...
  if (
    req.method !== "GET" &&
    req.method !== "HEAD" &&
    req.headers["x-dzmoney-admin-request"] !== "1"
  ) {
...
async function creditReferral(client, userId, earnedBux) {
  ...
  const referralCoins = referralBux * ECONOMY.COINS_PER_BUX;
  await client.query(
    `UPDATE users
     SET bux=bux+$1,
         coins=coins+$2,
...
app.post("/api/admin/login", rateLimit({ windowMs: 15 * 60 * 1000, max: 5, keyFn: req => req.ip }), async (req, res) => {
  if (!ADMIN_PASSWORD) {
    return res.status(503).json({
      success: false,
...
app.post("/api/admin/users/:id/balance", requireAdmin, async (req, res) => {
  ...
  if (!Number.isSafeInteger(buxDelta) || !Number.isSafeInteger(coinsDelta)) {
...
app.put("/api/admin/users/:id/balance", requireAdmin, async (req, res) => {
  ...
  if (!Number.isSafeInteger(bux) || !Number.isSafeInteger(coins) || bux < 0 || coins < 0) {
    return res.status(400).json({
      success: false,
...
app.post("/api/ton-proof/verify", requireSystemEnabled, requireTelegramAuth, async (req, res) => {
  try {
    ...
    try {
      ...
      // Prevent the same wallet from being bound to two accounts during
      // concurrent verification requests.
      await client.query("SELECT pg_advisory_xact_lock(hashtext($1))", [normalizedAddress]);
      ...
      // Consume the nonce only after all cryptographic and ownership checks
      // have succeeded.
      const consumed = await client.query(
        `DELETE FROM ton_proof_nonces
         WHERE id=$1 AND user_id=$2 AND used_at IS NULL
         RETURNING id`,
        [nonce.rows[0].id, userId]
      );
      ...
      if (!consumed.rowCount) {
        ...
        return res.status(409).json({
          success: false,
          message: "TON proof has already been used."
        });
      }
...
app.post("/api/withdrawals", requireSystemEnabled, requireTelegramAuth, async (req, res) => {
  ...
  try {
    ...
    const buxPerTon = ECONOMY.BUX_PER_TON;
...
app.put("/api/admin/withdrawals/:id", requireAdmin, async (req, res) => {
  ...
  try {
    ...
    if (nextStatus === w.status) {
      await client.query("ROLLBACK");
      return res.status(400).json({ success: false, message: "Withdrawal is already in this status." });
    }
    ...
    if (!validTransitions[w.status]?.has(nextStatus)) {
      await client.query("ROLLBACK");
      return res.status(400).json({
        success: false,
        message: `Invalid withdrawal transition: ${w.status} -> ${nextStatus}.`
      });
    }
...
app.post("/api/admin/tasks", requireAdmin, async (req, res) => {
  ...
  if (!id || !title || !Number.isSafeInteger(reward) || reward < 0) {
    return res.status(400).json({
      success: false,
...
app.put("/api/admin/tasks/:id", requireAdmin, async (req, res) => {
  ...
  if (!title || !Number.isSafeInteger(reward) || reward < 0) {
    return res.status(400).json({
      success: false,
      message: "Invalid task data."
    });
  ...
  try {
    ...
    for (const [rawKey, rawValue] of Object.entries(values)) {
      ...
      if (!allowedSettings.has(key)) {
        return res.status(400).json({
          success: false,
          message: `Unknown or protected setting: ${key}`
        });
      }

      if (key === "withdrawal_fee_bux" && (!/^\d+$/.test(value) || !Number.isSafeInteger(Number(value)))) {
        return res.status(400).json({
          success: false,
          message: "Withdrawal fee must be a non-negative whole number of BUX."
        });
      }

      if (key === "minimum_withdraw_bux" && (!/^\d+$/.test(value) || !Number.isSafeInteger(Number(value)) || Number(value) < 1)) {
        return res.status(400).json({
          success: false,
          message: "Minimum withdrawal must be a positive whole number of BUX."
        });
      }

      if (key === "referral_percentage") {
        const percentage = Number(value);
        if (!/^\d+(\.\d+)?$/.test(value) || percentage < 0 || percentage > 100) {
          return res.status(400).json({
            success: false,
      ...
      if (["daily_reward_bux", "daily_reward_coins", "daily_ads_limit"].includes(key) && !/^\d+$/.test(value)) {
        return res.status(400).json({
          success: false,
...
app.post("/api/telegram/webhook", async (req, res) => {
  ...
  if (!TELEGRAM_WEBHOOK_SECRET) {
    return res.status(503).json({
      success: false,
      message: "TELEGRAM_WEBHOOK_SECRET is required."
    });
  }

  if (getTelegramWebhookSecret(req) !== TELEGRAM_WEBHOOK_SECRET) {
    return res.status(401).json({
      success: false,
...
app.get("/api/user", requireTelegramAuth, requireSystemEnabled, async (req, res) => {
  try {
...
app.get("/api/tasks", requireTelegramAuth, requireSystemEnabled, async (req, res) => {
  try {
    const userId = getUserId(req);
    ...
    const result = await pool.query(
      `SELECT t.*, c.claimed_at, c.started_at
       FROM tasks t
    ...
    res.json({
      success: true,
      tasks: result.rows.map(task => {
        const claimedAt = task.claimed_at == null ? 0 : Number(task.claimed_at);
        const startedAt = task.started_at == null ? 0 : Number(task.started_at);
        const durationMs = Math.max(0, Number(task.duration) || 0) * 1000;
        const startRemaining =
          startedAt && !claimedAt && durationMs > 0
        ...
        if (claimedAt > 0) {
          completed = task.repeatable
            ? cooldownActive(claimedAt, Number(task.cooldown_seconds))
            : true;
        }
...
// Start task
// ============================
app.post("/api/tasks/:taskId/start", requireTelegramAuth, requireSystemEnabled, rateLimit({
  windowMs: 60 * 1000,
  max: 30,
  keyFn: req => `task-start:${req.telegramUser.id}`
}), async (req, res) => {
  const client = await pool.connect();
  ...
  try {
    ...
    const previous = claimResult.rowCount ? claimResult.rows[0] : null;

    if (previous && !task.repeatable && Number(previous.claimed_at) > 0) {
      await client.query("ROLLBACK");
      return res.status(400).json({
        success: false,
    ...
    if (
      previous &&
      task.repeatable &&
      Number(previous.claimed_at) > 0 &&
      cooldownActive(Number(previous.claimed_at), Number(task.cooldown_seconds))
    ) {
      const remaining = Math.ceil(
        (Number(task.cooldown_seconds) * 1000 - (now - Number(previous.claimed_at))) / 1000
      );
      await client.query("ROLLBACK");
      return res.status(400).json({
        success: false,
    ...
    const startedAt = Number(previous?.started_at || 0);
    if (startedAt && durationMs > 0) {
      ...
      if (elapsed < durationMs) {
        ...
        return res.json({
          ...
          alreadyStarted: true,
          startedAt,
          remaining: Math.max(0, Math.ceil((durationMs - elapsed) / 1000)),
          duration: Number(task.duration) || 0
        });
      }

      // The server-side timer has already elapsed. Do not reset it; let the
      // client immediately request the claim.
      await client.query("COMMIT");
      return res.json({
        success: true,
        alreadyStarted: true,
    ...
    if (previous) {
      await client.query(
        `UPDATE task_claims SET started_at=$1
    ...
    res.json({
      success: true,
      startedAt: now,
      remaining: Math.max(0, Math.ceil(durationMs / 1000)),
      duration: Number(task.duration) || 0
    });
  } catch (error) {
    await client.query("ROLLBACK").catch(() => {});
    console.error("Task start error:", error);
    res.status(500).json({ success: false, message: "Unable to start task." });
  } finally {
    client.release();
  }
});
...
// Claim task
// ============================
app.post("/api/tasks/:taskId/claim", requireTelegramAuth, requireSystemEnabled, rateLimit({ windowMs: 60 * 1000, max: 30, keyFn: req => `task:${req.telegramUser.id}` }), async (req, res) => {
  const client = await pool.connect();
  ...
  try {
    ...
    if (!Number.isSafeInteger(reward) || reward < 0) {
      ...
      return res.status(500).json({
        success: false,
        message: "Task reward configuration is invalid."
      });
    }
    ...
    if (durationMs > 0 && now - startedAt < durationMs) {
      const remaining = Math.ceil((durationMs - (now - startedAt)) / 1000);
      await client.query("ROLLBACK");
      return res.status(400).json({
        success: false,
        code: "TASK_NOT_COMPLETE",
        message: "Task duration has not elapsed yet.",
        remaining
      });
    }

    const coins = reward * ECONOMY.COINS_PER_BUX;
    ...
    if (previous) {
      await client.query(
        `UPDATE task_claims
         SET claimed_at=$1,started_at=0,reward=$2
...
app.post("/api/daily/claim", requireTelegramAuth, requireSystemEnabled, rateLimit({ windowMs: 60 * 1000, max: 5, keyFn: req => `daily:${req.telegramUser.id}` }), async (req, res) => {
  const client = await pool.connect();
...
// ============================
// API 404/error responses
// ============================
// Keep API failures JSON so the Telegram WebApp can always display a useful
// error instead of trying to parse an empty/plain-text response.
app.use((req, res, next) => {
