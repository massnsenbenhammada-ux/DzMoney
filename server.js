...
const referralRoutes = require('./src/http/referral-routes');
...
app.get('/', (_req, res) => {
  const html = indexHtml.replaceAll('__ASSET_VERSION__', assetVersion).replaceAll('__MONETAG_SCRIPTS__', monetagScriptsForClient().replaceAll('__ASSET_VERSION__', assetVersion)).replaceAll('__AD_PROVIDER_CONFIG__', JSON.stringify(clientAdConfig()).replace(/</g, '\\u003c'));
