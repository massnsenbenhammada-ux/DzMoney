...
- The membership becomes **ACTIVE only after the user performs at least one Verified Activity**.
...
- The user must perform **one Verified Activity** after purchase; then the membership becomes **ACTIVE**.
...
The default is **10 verified advertisements per new UTC+1 day**.
...
For Squad accounting:

**1 DZP earned = 1 DZP Contribution.**

Contribution is an accounting value for the relevant day/challenge period. It does not mint additional DZP.

The system must distinguish activity types when a Challenge scope requires it. For example, an advertisement Challenge does not count task-completion DZP, and a task Challenge does not count advertisement DZP.
...
Therefore, with a base reward of `1000 COIN + 1 DZX + 1 DZP` and a 15% Modifier:
...
Challenge rewards are credited to the users' balances; they do not create a second wallet or reward system.
