...
No new database table or migration is required. A separate orchestration service is justified only if the existing generic ad-event and provider boundaries cannot own the complete ad-to-reward transaction without mixing responsibilities; it must reuse those existing primitives rather than duplicate them.
