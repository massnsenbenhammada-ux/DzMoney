...
async function getPaidMembershipTiers(client = null) {
  ...
  const result = await execute(
    "SELECT value FROM admin_settings WHERE key = 'squad.membership_tiers'",
  );
  ...
  if (
    ...
    normalized.some(
      (tier) =>
        !Number.isInteger(tier.minMembers) ||
        !Number.isInteger(tier.maxMembers) ||
        tier.minMembers < 1 ||
        tier.maxMembers < tier.minMembers ||
...
async function activateOnVerifiedActivity(client, userId) {
  const result = await client.query(
    `UPDATE squad_memberships SET status = 'active' WHERE user_id = $1 AND status = 'inactive' RETURNING id, squad_id, user_id, status`,
    [userId],
  );
...
async function selectEligibleSquad(client, tier) {
  const result = await client.query(
    `SELECT s.id FROM squads s WHERE s.id = (SELECT candidate.id FROM (SELECT s2.id, COUNT(sm.id)::int AS member_count FROM squads s2 LEFT JOIN squad_memberships sm ON sm.squad_id = s2.id AND sm.status <> 'cancelled' GROUP BY s2.id HAVING COUNT(sm.id) BETWEEN $1 AND $2 ORDER BY COUNT(sm.id) ASC, s2.id ASC LIMIT 1) candidate) FOR UPDATE`,
...
async function purchasePaidMembership({ userId, maxMembers, idempotencyKey }) {
  ...
  return withTransaction(async (client) => {
    ...
    if (existingTransaction.rowCount) {
      ...
      if (
        !requestedTier ||
        Number(metadata.price) !== requestedTier.price ||
    ...
    const existingMembership = await client.query(
      `SELECT id FROM squad_memberships WHERE user_id = $1 AND status <> 'cancelled' FOR UPDATE`,
      [userId],
    );
