...
async function getPaidMembershipTiers(client) {
  const result = await client.query("SELECT value FROM admin_settings WHERE key = 'squad.membership_tiers'");
  ...
  const result = await client.query(
    `SELECT s.id
     ...
     WHERE s.id = (
       SELECT candidate.id
       FROM (
         SELECT s2.id, COUNT(sm.id)::int AS member_count
         FROM squads s2
         LEFT JOIN squad_memberships sm ON sm.squad_id = s2.id AND sm.status <> 'cancelled'
         GROUP BY s2.id
         HAVING COUNT(sm.id) BETWEEN $1 AND $2
         ORDER BY COUNT(sm.id) ASC, s2.id ASC
         LIMIT 1
       ) candidate
     )
     FOR UPDATE`,
