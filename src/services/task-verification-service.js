...
const { qualifyReferralForActivityOnClient } = require('./referral-service');
...
function resolveTrustedTaskVerifier({ config, telegramUserId, botToken = process.env.BOT_TOKEN, verifyMembership = isTelegramChannelMember }) {
  ...
  requiredId(botToken, 'BOT_TOKEN'); requiredId(telegramUserId, 'telegramUserId');
...
async function startTaskVerificationAd({ attemptId, idempotencyKey, externalAdId = null, providerRegistry, providerId = null }) {
  requiredId(attemptId, 'attemptId'); requiredId(idempotencyKey, 'idempotencyKey'); if (!providerRegistry) throw new Error('A trusted advertisement provider registry is required');
  ...
  return withTransaction(async client => {
    ...
    if (!gateResult.rowCount) throw new Error('Verification gate not found'); const gate = gateResult.rows[0];
    ...
    const resolvedExternalAdId = externalAdId || randomUUID(); const metadata = { attempt_id: attemptId, required_seconds: gate.required_seconds, provider_id: provider.id };
...
async function verifyTaskAdvertisement({ adEventId, providerRegistry, providerId = null, providerPayload }) {
  requiredId(adEventId, 'adEventId'); if (!providerRegistry) throw new Error('A trusted advertisement provider registry is required');
  ...
  const recordedProviderId = eventResult.rows[0].metadata?.provider_id; if (!recordedProviderId) throw new Error('Verification advertisement provider is not recorded');
...
async function finalizeTaskVerification({ attemptId, idempotencyKey, verifyTaskCompletion }) {
  ...
  return withTransaction(async client => {
    ...
    if (!result.rowCount) throw new Error('Task attempt not found'); const row = result.rows[0];
    if (row.status === 'verified') return { duplicate: true, status: 'verified' }; if (row.status !== 'verification_pending') throw new Error('Task attempt is not pending verification'); if (row.gate_status !== 'ad_completed') throw new Error('Verification advertisement must be verified first');
    const resolvedConfig = resolveVerificationConfig({ taskType: row.task_type, config: row.config }); const completion = resolvedConfig.completion; let verifiedByTaskRule;
    if (completion.mode === 'open_link') { if (row.metadata?.link_clicked !== true) return { duplicate: false, status: 'verification_pending', rewarded: false, reason: 'link_click_required' }; verifiedByTaskRule = true; }
    else { const verifier = verifyTaskCompletion || resolveTrustedTaskVerifier({ config: resolvedConfig, telegramUserId: row.telegram_user_id }); verifiedByTaskRule = await verifier({ attemptId }); if (typeof verifiedByTaskRule !== 'boolean') throw new Error('Task verifier must return a boolean'); }
    if (verifiedByTaskRule !== true) { await client.query(`UPDATE task_attempts SET status='rejected',rejected_at=NOW() WHERE id=$1`, [attemptId]); await client.query(`UPDATE task_verification_gates SET status='rejected' WHERE id=$1`, [row.gate_id]); return { duplicate: false, status: 'rejected', rewarded: false }; }
    const baseReward = { coin: Number(row.reward_coin), dzx: Number(row.reward_dzx), dzp: Number(row.reward_dzp) };
    const reward = await creditActivityRewardOnClient(client, { idempotencyKey, userId: row.user_id, source: 'task', ...baseReward, modifiers: [] });
    const referral = await qualifyReferralForActivityOnClient(client, { referredUserId: row.user_id, activityId: `task:${attemptId}`, baseReward });
    ...
    return { duplicate: false, status: 'verified', rewarded: true, reward, referral };
  });
}
