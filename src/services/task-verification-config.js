...
function validateReferral(referral = {}) {
  ...
  if (!REFERRAL_MODES.includes(mode))
    throw new Error("Invalid external referral mode");
  if (mode !== "disabled" && !referral.referralUrlTemplate)
    throw new Error("referralUrlTemplate is required");
...
function rejectSecrets(value) {
  ...
  for (const [key, child] of Object.entries(value)) {
    if (SECRET_KEYS.has(key))
      throw new Error("credentials must not be stored in task config");
    rejectSecrets(child);
  }
}
...
function validateProviderEvidence(verification = {}) {
  ...
  if (!hasProvider) {
    ...
    if (hasEvent)
      throw new Error(
        "provider is required when a verification event is configured",
      );
    return;
  }
  if (
    ...
    throw new Error("verification method is required");
  if (!hasEvent || typeof verification.event !== "string")
    throw new Error("verification event is required");
  if (
    verification.providerConfigRef !== undefined &&
    verification.providerConfigRef !== null &&
    typeof verification.providerConfigRef !== "string"
  )
    throw new Error("providerConfigRef must be a string");
}

function validateCreatorVerificationContract(
  taskType,
  verification = {},
  campaignUrl = null,
) {
  if (!CREATOR_TASK_TYPES.includes(taskType))
    throw new Error("Invalid creator task type");
  const allowed = CREATOR_VERIFICATION_METHODS[taskType];
  if (!allowed.includes(verification.method))
    throw new Error(`Invalid verification method for ${taskType} creator task`);
  if (typeof campaignUrl !== "string" || campaignUrl.trim() === "")
    throw new Error("campaignUrl is required for creator tasks");
  if (verification.method === "click_proof") {
    if (
      verification.provider ||
      verification.event ||
      verification.providerConfigRef ||
      verification.requirements
    )
      throw new Error("click_proof does not use a provider contract");
    return;
  }
  if (verification.method === "url_format_match") {
    if (taskType !== "game")
      throw new Error("url_format_match is supported only for Game tasks");
    if (
      verification.provider ||
      verification.event ||
      verification.providerConfigRef ||
      ...
      throw new Error("url_format_match does not use a provider contract");
    return;
  }
  if (verification.method !== "bot_api" || taskType !== "social")
    throw new Error("Invalid creator verification contract");
  const provider = CREATOR_PROVIDER_CONTRACTS.social[0];
  if (verification.provider !== provider.id)
    throw new Error("Creator Bot API provider is not enabled");
  if (verification.event !== provider.event)
    throw new Error(
      "Creator Bot API event does not match the provider contract",
    );
...
function validateCreatorProviderRequirements(provider, requirements = {}) {
  ...
  if (provider.id === "telegram_channel") {
    if (
      typeof requirements.channel !== "string" ||
      !TELEGRAM_CHANNEL_PATTERN.test(requirements.channel.trim())
    )
      throw new Error("Invalid Telegram channel requirement");
...
function validateCreatorProviderConfiguration(taskType, config = {}) {
  if (Object.prototype.hasOwnProperty.call(config, "completion"))
    throw new Error("Legacy Creator completion contract is not supported");
...
function validateVerificationConfig(config = {}, taskType = null) {
  ...
  if (Object.prototype.hasOwnProperty.call(config, "completion"))
    throw new Error("Legacy completion configuration is not supported");
  const verification = config.verification || {};
  ...
  if (taskType === "game" && verification.method === "url_format_match") {
    if (
      typeof config.campaignUrl !== "string" ||
      config.campaignUrl.trim() === ""
    )
...
function resolveVerificationConfig({ taskType, config = {} }) {
  ...
  return {
    ...
    achievementThreshold:
      source.achievementThreshold === undefined
        ? null
        : Number(source.achievementThreshold),
    ...
    verification: {
      ...
      requirements:
        verification.requirements &&
        typeof verification.requirements === "object"
          ? { ...verification.requirements }
          : {},
    },
    referral: {
