...
async function getDailyCheckinSettings(client) {
  const cooldownHours = await settingNumber(
    client,
    'activity.daily_checkin_cooldown_hours',
    DEFAULT_COOLDOWN_HOURS,
  );
  const coin = await settingNumber(
    client,
    'activity.daily_checkin_reward_coin',
    DEFAULT_REWARD.coin,
  );
  const dzx = await settingNumber(
    client,
    'activity.daily_checkin_reward_dzx',
    DEFAULT_REWARD.dzx,
  );
  const dzp = await settingNumber(
    client,
    'activity.daily_checkin_reward_dzp',
    DEFAULT_REWARD.dzp,
  );
...
async function getDailyCheckinStatus({ userId }) {
  ...
  if (state.last_claimed_at) {
    ...
    const nextEligibleAt = new Date(
      new Date(state.last_claimed_at).getTime() +
...
async function startDailyCheckinClaim({
  userId,
  idempotencyKey,
  providerRegistry,
  providerId = null,
  ...
  return withTransaction(async (client) => {
    ...
    const state = await client.query(
      'SELECT * FROM daily_checkins WHERE user_id=$1 FOR UPDATE',
      [userId],
    );
    const existing = state.rows[0];
    if (existing?.last_claimed_at) {
      const nextEligibleAt = new Date(
        new Date(existing.last_claimed_at).getTime() +
          settings.cooldownHours * 3600000,
      );
      if (nextEligibleAt.getTime() > Date.now()) {
    ...
    if (existing?.ad_event_id && !existing.last_claimed_at) {
      const event = await client.query(
        'SELECT * FROM activity_ad_events WHERE id=$1',
        [existing.ad_event_id],
      );
      if (event.rowCount && !event.rows[0].verified)
        return {
          claimIdempotencyKey: existing.claim_idempotency_key,
          adEvent: event.rows[0],
          providerId: event.rows[0].metadata?.provider_id,
          duplicate: true,
        };
    }
...
async function verifyDailyCheckinAd({
  userId,
  adEventId,
  providerRegistry,
  providerId = null,
  ...
  const eventResult = await query(
    'SELECT user_id,context,metadata FROM activity_ad_events WHERE id=$1',
    [adEventId],
  );
  if (!eventResult.rowCount || eventResult.rows[0].context !== 'daily_checkin')
    throw new Error('Daily Check-in advertisement event not found');
  if (String(eventResult.rows[0].user_id) !== String(userId))
    throw new Error('Daily Check-in advertisement does not belong to the user');
  const recordedProviderId = eventResult.rows[0].metadata?.provider_id;
  if (!recordedProviderId)
    throw new Error('Daily Check-in advertisement provider is not recorded');
  if (providerId && providerId !== recordedProviderId)
    throw new Error(
      'Advertisement provider does not match the selected provider',
    );
  getProviderForVerification(providerRegistry, {
    context: 'daily_checkin',
...
async function finalizeDailyCheckin({ userId, claimIdempotencyKey }) {
  ...
  return withTransaction(async (client) => {
    ...
    if (!stateResult.rowCount)
      throw new Error('Daily Check-in claim not found');
    const state = stateResult.rows[0];
    if (state.claim_idempotency_key !== claimIdempotencyKey)
      throw new Error(
        'Daily Check-in claim does not match the current ad gate',
      );
    if (state.last_claimed_at) {
      const settings = await getDailyCheckinSettings(client);
      const nextEligibleAt = new Date(
      ...
      if (nextEligibleAt.getTime() > Date.now())
        return { duplicate: true, rewarded: false, nextEligibleAt };
    }
    const adResult = await client.query(
      'SELECT * FROM activity_ad_events WHERE id=$1 FOR UPDATE',
      [state.ad_event_id],
    );
    if (!adResult.rowCount || adResult.rows[0].context !== 'daily_checkin')
      throw new Error('Daily Check-in advertisement event not found');
    if (String(adResult.rows[0].user_id) !== String(userId))
      throw new Error(
        'Daily Check-in advertisement does not belong to the user',
      );
    if (!adResult.rows[0].verified)
      throw new Error('Daily Check-in advertisement must be verified first');
    const settings = await getDailyCheckinSettings(client);
    ...
    const updated = await client.query(
      'UPDATE daily_checkins SET last_claimed_at=NOW(),updated_at=NOW() WHERE user_id=$1 RETURNING *',
      [userId],
    );
