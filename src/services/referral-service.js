...
async function resolveExistingAttribution(
  client,
  referrerUserId,
  referredUserId,
) {
...
async function verifyQualificationEvidence(
  client,
  referredUserId,
  source,
  referenceId,
...
async function qualifyReferral({
  ...
  return withTransaction(async (client) => {
    ...
    if (!updated.rowCount)
      return {
        attribution: await findAttribution(client, referred),
        duplicate: true,
      };
...
async function creditReferralLifetimeOnClient(
  client,
  ...
  const reward = await postEconomyTransactionOnClient(client, {
    ...
    metadata: {
      ...
      base_reward: {
        coin: Number(baseReward?.coin || 0),
        dzx: Number(baseReward?.dzx || 0),
      },
    },
    movements,
  });
...
async function creditReferralLifetime(args) {
  return withTransaction((client) =>
    creditReferralLifetimeOnClient(client, args),
  );
}
...
module.exports = {
  ...
  qualifyReferral,
  activateReferral,
  creditReferralLifetime,
  creditReferralLifetimeOnClient,
