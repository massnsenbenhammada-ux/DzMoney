...
async function getDepositSettings() {
  const result = await query(
    `SELECT key,value FROM admin_settings WHERE key LIKE 'deposit.%' ORDER BY key`,
  );
...
async function expireStalePendingDeposits(client, userId, timeoutHours) {
  await client.query(
    `UPDATE deposits SET status='REJECTED',metadata=metadata || jsonb_build_object('rejection_reason','PENDING_TIMEOUT','rejected_at',NOW()),updated_at=NOW() WHERE user_id=$1 AND status='PENDING' AND created_at <= NOW() - ($2::numeric * INTERVAL '1 hour')`,
    [userId, timeoutHours],
  );
}
async function reserveDailyDepositQuota(client, userId, tonAmount) {
  ...
  const result = await client.query(
    `UPDATE deposit_daily_usage SET ton_used=ton_used+$2::numeric,updated_at=NOW() WHERE user_id=$1 AND usage_date=CURRENT_DATE AND ton_used+$2::numeric <= $3::numeric RETURNING ton_used,GREATEST($3::numeric-ton_used,0) AS remaining`,
...
async function processDeposit({
  ...
  return withTransaction(async (client) => {
    ...
    const inserted = await client.query(
      `INSERT INTO deposits (idempotency_key,user_id,blockchain,network,tx_hash,ton_amount,dzx_amount,confirmation_count,required_confirmations,status,metadata,confirmed_at) VALUES ($1,$2,'TON',$3,$4,$5,$6,0,$7,'PENDING',$8,NULL) ON CONFLICT (idempotency_key) DO NOTHING RETURNING *`,
      [
        idempotencyKey,
        userId,
        network,
...
async function verifyStoredDeposit(deposit) {
  ...
  if (!configured)
    throw new Error(`TON ${deposit.network} deposit address is not configured`);
  const expectedDestination = normalizeTonAddress(
    configured.address || configured,
  );
  return verifyTonDeposit({
    network: deposit.network,
    txHash: deposit.tx_hash,
    expectedAmountTon: deposit.ton_amount,
    expectedDestination,
    apiKey: process.env.TONCENTER_API_KEY,
    baseUrl: process.env.TONCENTER_API_BASE_URL,
  });
}
  ...
  return withTransaction(async (client) => {
    ...
    if (
      ...
      const rejected = await client.query(
        `UPDATE deposits SET status='REJECTED',metadata=metadata || jsonb_build_object('rejection_reason','PENDING_TIMEOUT','rejected_at',NOW()),updated_at=NOW() WHERE id=$1 RETURNING *`,
        [deposit.id],
      );
      return {
        deposit: rejected.rows[0],
        duplicate: false,
        credited: false,
        expired: true,
      };
    ...
    const updated = await client.query(
      `UPDATE deposits SET status='CONFIRMED',confirmation_count=$1,confirmed_at=NOW(),metadata=metadata || $2::jsonb,updated_at=NOW() WHERE id=$3 RETURNING *`,
      [
        ...
        JSON.stringify({
          ...
          blockchain_finality: verification.finality,
          blockchain_network: verification.network,
          masterchain_seqno: verification.masterchainSeqno,
        }),
        deposit.id,
      ],
    );
