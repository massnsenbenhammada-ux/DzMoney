...
async function creditConfirmedDeposit(client, deposit, verification, extraMetadata = {}) {
  return postEconomyTransactionOnClient(client, {
    ...
    type: 'DEPOSIT',
    metadata: { source:'deposit', deposit_id:deposit.id, blockchain:deposit.blockchain, tx_hash:deposit.tx_hash, ton_amount:deposit.ton_amount, dzx_amount:deposit.dzx_amount, confirmation_count:deposit.confirmation_count, required_confirmations:deposit.required_confirmations, blockchain_finality:verification.finality, blockchain_network:verification.network, ...extraMetadata },
...
async function verifyStoredDeposit(deposit) {
  ...
  return verifyTonDeposit({ network:deposit.network, txHash:deposit.tx_hash, expectedAmountTon:deposit.ton_amount, expectedDestination, apiKey:process.env.TONCENTER_API_KEY });
}
async function confirmDeposit({ idempotencyKey, metadata = {} }) {
  ...
  const depositBeforeVerification = initial.rows[0];
  if (depositBeforeVerification.status === 'REJECTED') throw new Error('Rejected deposit cannot be confirmed');
  if (depositBeforeVerification.status === 'CONFIRMED') return { deposit:depositBeforeVerification, duplicate:true, credited:true };
  const verification = await verifyStoredDeposit(depositBeforeVerification);
  return withTransaction(async client => {
    ...
    if (deposit.tx_hash !== depositBeforeVerification.tx_hash || deposit.network !== depositBeforeVerification.network || deposit.ton_amount !== depositBeforeVerification.ton_amount) throw new Error('Deposit changed during blockchain verification');
    const timeoutHours = await settingNumber(client, 'deposit.pending_timeout_hours', 24);
