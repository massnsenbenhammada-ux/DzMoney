...
async function getUserProfile(userId) {
  ...
  const [wallets, ledger] = await Promise.all([
    query(`SELECT currency, balance, earned_dzp, converted_dzp, purchased_dzp
           FROM wallet_accounts WHERE user_id = $1 ORDER BY currency`, [userId]),
    query(`SELECT le.id, le.currency, le.amount, le.balance_before, le.balance_after,
                  le.source, le.created_at, lt.id AS transaction_id, lt.transaction_type,
                  lt.idempotency_key, lt.metadata
           FROM ledger_entries le
           JOIN ledger_transactions lt ON lt.id = le.transaction_id
           WHERE lt.user_id = $1
           ORDER BY le.id DESC LIMIT 50`, [userId]),
  ]);
...
module.exports = { searchUsers, getUserProfile, adjustBalance, validateAdjustment, requireReason };
