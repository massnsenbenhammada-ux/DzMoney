...
function nextState(event, field) { return { ...(event.metadata?.provider_state || {}), [field]: true }; }
...
async function finalizeIfReady(client, event, state) {
  ...
  const result = await client.query(`UPDATE activity_ad_events SET completed_at=COALESCE(completed_at,NOW()), verified=TRUE, metadata=metadata || $2::jsonb WHERE id=$1 AND verified=FALSE RETURNING *`, [event.id, JSON.stringify({ provider_reference: reference, provider_verification: { provider_id: 'adsgram', block_id: ADSGRAM_BLOCK_ID, client_completed: true, provider_confirmed: true, source: 'adsgram_dual_confirmation' } })]);
  return { ready: true, rewarded: false, adEvent: result.rows[0] || event };
}
