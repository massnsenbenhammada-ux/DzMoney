...
async function markClientStarted({ userId, adEventId }) {
  return withTransaction(async client => {
    ...
    if (event.metadata?.provider_state?.client_started === true) return { started: true, duplicate: true, adEvent: event };
...
async function markClientCompleted({ userId, adEventId }) {
  return withTransaction(async client => {
    ...
    if (current.client_started !== true) throw new Error('AdsGram advertisement has not started');
    const state = nextState(event, 'client_completed');
    if (current.client_completed === true) {
      return {
        ready: state.client_completed === true && state.provider_confirmed === true,
        rewarded: false,
        adEvent: event,
        duplicate: true
      };
...
async function markProviderConfirmed({ userTelegramId, providerReference }) {
  return withTransaction(async client => {
    ...
    if (!result.rowCount) throw new Error('No started AdsGram advertisement matches the provider callback');
    const event = result.rows[0];
    const current = event.metadata?.provider_state || {};
    if (current.provider_confirmed === true) {
      return { ready: current.client_completed === true, rewarded: false, adEvent: event, duplicate: true };
    }
    const state = nextState(event, 'provider_confirmed');
    const updated = await client.query(
      'UPDATE activity_ad_events SET metadata=metadata || $2::jsonb WHERE id=$1 RETURNING *',
      [event.id, JSON.stringify({ provider_reference: String(providerReference), provider_state: state })]
    );
    return finalizeIfReady(client, updated.rows[0], state);
  });
}
...
async function finalizeIfReady(client, event, state) {
  if (!(state.client_started === true && state.client_completed === true && state.provider_confirmed === true)) {
  ...
  return { ready: true, rewarded: false, adEvent: result.rows[0] || event, duplicate: false };
}

module.exports = { markClientStarted, markClientCompleted, markProviderConfirmed, ADSGRAM_BLOCK_ID };
