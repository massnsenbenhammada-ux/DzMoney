...
function matchesUrlFormat(referenceUrl, candidateUrl) {
  ...
  if (
    ...
    reference.hostname !== candidate.hostname ||
    reference.port !== candidate.port ||
    reference.pathname !== candidate.pathname ||
    reference.hash !== candidate.hash
  )
    return false;
  ...
  return referenceEntries.every(
    ([key, value], index) =>
      candidateEntries[index][0] === key &&
      matchesReferralValue(value, candidateEntries[index][1]),
  );
}
