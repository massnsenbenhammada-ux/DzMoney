...
const {
  ...
  DAILY_SYSTEM_TASKS,
} = require('./daily-system-task-contract');
...
async function getSystemTask(systemKey) {
  ...
  const result = await query(
    `SELECT * FROM activity_tasks WHERE task_type='daily' AND status='active' AND config->>'systemKey'=$1 ORDER BY id LIMIT 1`,
    [systemKey],
  );
...
async function getLatestDailyCompletion(task, userId) {
  const result = await query(
    `SELECT GREATEST((SELECT MAX(verified_at) FROM task_attempts WHERE task_id=$1 AND user_id=$2 AND status='verified'),(SELECT last_claimed_at FROM daily_checkins WHERE user_id=$2)) AS completed_at`,
...
async function assertAvailable(task, userId, now = new Date()) {
  ...
  if (policy === 'rolling_24h')
    return isRolling24HourAvailable(completedAt, now);
  if (policy === 'utc_plus_one_calendar_day')
    return isUtcPlusOneCalendarDayAvailable(completedAt, now);
...
async function getAdvertisementProgress(task, userId) {
  ...
  if (!Number.isInteger(target) || target <= 0)
    throw new Error('Invalid daily advertisement target');
  const result = await query(
    `SELECT COUNT(*)::int AS completed FROM activity_ad_events WHERE user_id=$1 AND context='task' AND verified=TRUE AND metadata->>'task_id'=$2 AND (completed_at + INTERVAL '1 hour')::date = (NOW() + INTERVAL '1 hour')::date`,
...
async function assertReferralAchievementAvailable(task, userId) {
  ...
  return isReferralAchievementClaimable(
    qualified,
    threshold,
    claimed.rowCount > 0,
  );
...
async function executeSystemTask({
  ...
  if (task.config?.achievementThreshold !== undefined) {
    if (!(await assertReferralAchievementAvailable(task, userId)))
      throw new Error('Referral achievement is not claimable');
  } else if (systemKey === DAILY_SYSTEM_TASKS.VIEW_ADS) {
    if (!(await assertAdvertisementAvailable(task, userId)))
      throw new Error('Daily advertisement target is already complete');
  } else if (!(await assertAvailable(task, userId))) {
    throw new Error(
      'Daily task is already completed for the current eligibility window',
    );
  }
...
module.exports = {
  ...
  assertAdvertisementAvailable,
  assertReferralAchievementAvailable,
  executeSystemTask,
};
