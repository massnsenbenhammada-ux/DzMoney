  ...
  return {
    ...
    async verifyCompletion(payload = {}) {
      const normalized = validateMonetagPostback({
        ...payload,
        zone_id: payload.zone_id || MONETAG_ZONE_ID,
      });
      return {
    ...
    async verifyServerCompletion(payload = {}) {
      const normalized = validateMonetagPostback(
        { ...payload, zone_id: payload.zone_id || MONETAG_ZONE_ID },
        MONETAG_TASK_CONTEXT,
      );
      if (!normalized.telegramId)
        return { verified: false, reference: normalized.ymid };
      return {
        verified: normalized.eligible,
        reference: normalized.ymid,
        userId: normalized.telegramId,
        providerId: MONETAG_PROVIDER_ID,
        context: MONETAG_TASK_CONTEXT,
        metadata: normalized,
      };
    },
  };
