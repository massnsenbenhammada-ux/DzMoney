...
async function getEconomySettings() {
  const result = await query(
    `SELECT key, value
     FROM admin_settings
     WHERE key LIKE 'economy.%'
        OR key LIKE 'activity.%'
        OR key LIKE 'reward_pool.%'
        OR key LIKE 'withdrawal.%'
     ORDER BY key`
  );
...
async function applyMovement(client, { userId, currency, amount, source, dzpBucket = null }) {
  ...
  await client.query(
    `UPDATE wallet_accounts SET ${updates.join(', ')} WHERE id = $2`,
    params
  );
...
async function convertCoinToDzp({ idempotencyKey, userId, coin }) {
  ...
  return withTransaction(async client => {
    ...
    const entries = [];
    for (const entry of [coinEntry, dzpEntry]) {
      const row = await client.query(
        `INSERT INTO ledger_entries
          (transaction_id, wallet_account_id, amount, balance_before, balance_after, source, currency)
...
async function convertDzxToDzp({ idempotencyKey, userId, dzx }) {
  ...
  return withTransaction(async client => {
    ...
    const dzpEntry = await applyMovement(client, { userId, currency: 'DZP', amount: dzp, source: 'conversion', dzpBucket: 'converted_dzp' });
    const entries = [];
    for (const entry of [dzxEntry, dzpEntry]) {
      const row = await client.query(
        `INSERT INTO ledger_entries
          (transaction_id, wallet_account_id, amount, balance_before, balance_after, source, currency)
         VALUES ($1,$2,$3,$4,$5,$6,$7) RETURNING *`,
