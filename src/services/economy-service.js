...
function multiplyScaled(left, right) {
  ...
  if (
    remainder !== 0n &&
    (remainder < 0n ? -remainder : remainder) * 2n >= DECIMAL_SCALE
  )
...
async function getEconomySettings() {
  const result = await query(
    `SELECT key, value FROM admin_settings WHERE key LIKE 'economy.%' OR key LIKE 'activity.%' OR key LIKE 'withdrawal.%' ORDER BY key`,
  );
...
async function postEconomyTransactionOnClient(
  client,
...
async function postEconomyTransaction(args) {
  return withTransaction((client) =>
    postEconomyTransactionOnClient(client, args),
  );
}
...
function normalizeActivityReward({ source, coin, dzx, dzp, modifiers }) {
  ...
  const baseScaled = Object.fromEntries(
    Object.entries(baseReward).map(([currency, amount]) => [
      currency,
      decimalToScaled(amount, currency),
    ]),
  );
  for (const [currency, amount] of Object.entries(baseScaled))
    if (amount < 0n)
...
async function creditActivityRewardOnClient(client, args) {
  const {
    ...
    dzp = 0,
    modifiers = [],
    qualifyingVerifiedActivity = false,
    activityDay = null,
    activityType = null,
...
async function creditActivityReward(args) {
  return withTransaction((client) =>
    creditActivityRewardOnClient(client, args),
  );
}
