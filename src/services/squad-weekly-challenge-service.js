...
function scaledDecimal(value) {
  ...
  if (fraction.length > 9)
    throw new Error("Decimal exceeds NUMERIC(30,9) precision");
...
function matchesChallengeScope(activity, scope, scopeValue) {
  ...
  if (scope === "ALL TASKS" || scope === "Verified Task")
    return activity.source === "task";
  if (scope === "Type Tasks")
    return activity.source === "task" && activity.activityType === scopeValue;
  if (scope === "Verified Ad") return activity.source === "advertisement";
  if (scope === "Verified Squad AdView")
    return (
      activity.source === "advertisement" &&
      activity.activityContext === "squad"
    );
...
function distributeReward(totalReward, contributors) {
  ...
  const normalized = contributors
    .map((item) => ({
      ...
      contribution: scaledDecimal(item.contribution),
    }))
  ...
  let remainingContribution = normalized.reduce(
  ...
  return normalized.map((item, index) => {
    ...
    return {
      userId: item.userId,
      contribution: decimalFromScaled(item.contribution),
      rewardScaled: String(rewardScaled),
      rewardAmount: decimalFromScaled(rewardScaled),
    };
  });
}
...
function validateScope(scope, scopeValue) {
  if (!VALID_SCOPES.includes(scope))
    throw new Error("Unsupported challenge scope");
  if (scope === "Type Tasks" && !TASK_TYPES.includes(scopeValue))
    throw new Error("Unsupported task challenge type");
  if (
    scope !== "Type Tasks" &&
    scopeValue !== null &&
    ...
    throw new Error("scopeValue is only valid for Type Tasks");
}

function validateReward(currency, amount) {
  if (!CURRENCIES.includes(currency))
    throw new Error("Unsupported challenge reward currency");
  if (scaledDecimal(amount) <= 0n)
    throw new Error("Challenge reward amount must be positive");
}
...
async function settleWeeklyChallenge({ challengeId, now = new Date() }) {
  return withTransaction(async (client) => {
    const result = await client.query(
      "SELECT * FROM squad_weekly_challenges WHERE id=$1 FOR UPDATE",
      [challengeId],
    );
    ...
    if (challenge.status === "settled")
      return {
        duplicate: true,
        challenge,
        rewards: await existingRewards(client, challenge.id),
      };
    if (new Date(now) < new Date(challenge.ends_at))
      throw new Error("Challenge has not ended");
    ...
    const rewards = distributeReward(
      challenge.reward_amount,
      eligible.rows.map((row) => ({
        userId: row.user_id,
        contribution: row.dzp_contribution,
      })),
    );
    for (const reward of rewards) {
      ...
      const transaction = await postEconomyTransactionOnClient(client, {
        ...
        metadata: {
          ...
          challenge_scope: challenge.scope_type,
        },
        movements: [
          {
            currency: challenge.reward_currency,
    ...
    await client.query(
      "UPDATE squad_weekly_challenges SET status='settled',settled_at=NOW() WHERE id=$1",
      [challenge.id],
    );
...
async function existingRewards(client, challengeId) {
  const result = await client.query(
    "SELECT user_id,dzp_contribution,reward_amount,reward_transaction_id FROM squad_weekly_challenge_contributions WHERE challenge_id=$1 AND reward_amount IS NOT NULL ORDER BY user_id",
    [challengeId],
  );
  return result.rows;
}
...
async function listWeeklyChallenges({ squadId }) {
  ...
  const result = await query(
    `SELECT id,squad_id,name,scope_type,scope_value,reward_currency,reward_amount,starts_at,ends_at,status,config_snapshot,settled_at FROM squad_weekly_challenges WHERE squad_id=$1 ORDER BY starts_at DESC,id DESC`,
    [squadId],
  );
  return result.rows;
}
