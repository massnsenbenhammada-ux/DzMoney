...
async function refreshMembershipStatuses(client, squadId, now = new Date()) {
  ...
  await client.query(`UPDATE squad_memberships SET status='inactive',updated_at=NOW() WHERE squad_id=$1 AND status='active' AND ((last_activity_at IS NOT NULL AND last_activity_at<$2) OR (last_activity_at IS NULL AND joined_at<$2))`, [squadId, cutoff]);
...
async function addMember({ squadId, userId, parentUserId = null }) {
  required(squadId, 'squadId'); required(userId, 'userId');
  return withTransaction(async client => {
...
async function recordQualifyingActivity({ userId, activityType, activityId = null, quantity = 1, occurredAt = new Date(), idempotencyKey, metadata = {} }) {
  required(userId,'userId'); required(activityType,'activityType'); required(idempotencyKey,'idempotencyKey');
...
async function getMembershipForUser(telegramUserId) {
  const result = await query(`SELECT m.*,u.telegram_user_id,u.username,u.first_name FROM users u JOIN squad_memberships m ON m.user_id=u.id WHERE u.telegram_user_id=$1 AND m.status<>'removed'`, [required(telegramUserId,'telegramUserId')]);
...
async function getMembers(squadId) { return query(`SELECT * FROM squad_memberships WHERE squad_id=$1 ORDER BY joined_at,id`,[required(squadId,'squadId')]); }
...
async function getDailyEligibility({squadId,date=new Date()}) {
  ...
  return withTransaction(async client=>{
    ...
    const activeMembers=Number(counts.rows[0].active_member_count),activeToday=Number(counts.rows[0].active_today_count);
...
async function createGoal({squadId,title,description=null,targetType,targetQuantity,rewardPool,startsAt=new Date(),expiresAt=null,status='active'}) {
  ...
  const target=positiveNumber(targetQuantity,'targetQuantity'),pool=positiveNumber(rewardPool,'rewardPool');
  return withTransaction(async client=>{const result=await client.query(`INSERT INTO squad_goals(squad_id,title,description,target_type,target_quantity,reward_currency,reward_pool,starts_at,expires_at,status) VALUES($1,$2,$3,$4,$5,'DZX',$6,$7,$8,$9) RETURNING *`,[squadId,title,description,targetType,target,pool,startsAt,expiresAt,status]);return result.rows[0];});
}
...
async function recordGoalContribution({goalId,activityEventId,userId,quantity}) {
  ...
  return withTransaction(async client=>{
    ...
    if(!goalResult.rowCount) throw new Error('Goal not found'); const goal=goalResult.rows[0];
...
async function getGoalProgress(goalId) {
  ...
  if(!result.rowCount) throw new Error('Goal not found'); const row=result.rows[0],progress=Number(row.progress);
...
async function calculateGoalDistribution(goalId) {
  return withTransaction(async client=>{
    ...
    const totalWeight=contributions.rows.reduce((sum,row)=>sum+Number(row.weight),0); if(totalWeight<=0) throw new Error('Goal total weight must be positive');
    const pool=Number(goal.reward_pool);
    const distributions=contributions.rows.map(row=>{const weight=Number(row.weight),reward=pool*weight/totalWeight;return {userId:row.user_id,weight,totalWeight,rewardAmount:reward,calculation:{formula:'reward_pool * member_weight / total_weight',reward_pool:pool,member_weight:weight,total_weight:totalWeight}};});
...
async function snapshotGoalDistribution(goalId) {
  ...
  return withTransaction(async client=>{
    ...
    if(!goal.rowCount) throw new Error('Goal not found'); if(['cancelled','expired'].includes(goal.rows[0].status)) throw new Error('Goal cannot be rewarded');
    ...
    const inserted=[];
    for(const item of calculated.distributions){const result=await client.query(`INSERT INTO squad_goal_distributions(goal_id,user_id,weight,total_weight,reward_amount,calculation,idempotency_key) VALUES($1,$2,$3,$4,$5,$6,$7) RETURNING *`,[goalId,item.userId,item.weight,item.totalWeight,item.rewardAmount,item.calculation,`squad-goal:${goalId}:user:${item.userId}`]);inserted.push(result.rows[0]);}
...
async function getGoalContribution({goalId,userId}) {
  ...
  const row=result.rows[0],weight=Number(row.weight),quantity=Number(row.quantity),eventCount=Number(row.event_count);
