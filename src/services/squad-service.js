...
async function createSquad({ ownerUserId }) {
  ...
  return withTransaction(async client => {
    ...
    const squadResult = await client.query(
      `INSERT INTO squads (owner_user_id) VALUES ($1) RETURNING *`,
      [ownerUserId]
    );
    await client.query(
      `INSERT INTO squad_memberships (squad_id, user_id, parent_user_id, active_since)
       VALUES ($1, $2, NULL, NOW())`,
      [squadResult.rows[0].id, ownerUserId]
    );
...
async function recordQualifyingActivity({ userId, activityType, activityId = null, quantity = 1, occurredAt = new Date(), idempotencyKey, metadata = {} }) {
  ...
  return withTransaction(async client => {
    const membership = await client.query(
      `SELECT * FROM squad_memberships WHERE user_id = $1 AND status <> 'removed' FOR UPDATE`, [userId]
    );
    ...
    await client.query(
      `UPDATE squad_memberships
          SET last_activity_at = $1, active_since = CASE WHEN status = 'inactive' THEN $1 ELSE active_since END,
...
async function getDailyEligibility({ squadId, date = new Date() }) {
  ...
  return withTransaction(async client => {
    ...
    const counts = await client.query(
      `SELECT
         ...
         COUNT(*) FILTER (WHERE status = 'active' AND EXISTS (
           SELECT 1 FROM squad_activity_events e
            WHERE e.squad_id = m.squad_id AND e.user_id = m.user_id
              AND e.occurred_at >= $2::date
              AND e.occurred_at < ($2::date + INTERVAL '1 day')
         ))::int AS active_today_count
       FROM squad_memberships m
      WHERE m.squad_id = $1 AND m.status IN ('active','inactive')`,
      [squadId, date]
    );
    ...
    const bonusDate = day.toISOString().slice(0, 10);
    const saved = await client.query(
      `INSERT INTO squad_daily_bonus_days
        (squad_id,bonus_date,qualified,active_member_count,active_today_count,activity_percent,bonus_rate)
       VALUES ($1,$2,$3,$4,$5,$6,$7)
       ON CONFLICT (squad_id,bonus_date) DO UPDATE SET
         qualified=EXCLUDED.qualified, active_member_count=EXCLUDED.active_member_count,
         active_today_count=EXCLUDED.active_today_count, activity_percent=EXCLUDED.activity_percent,
         bonus_rate=EXCLUDED.bonus_rate, calculated_at=NOW()
       RETURNING *`,
      [squadId, bonusDate, qualified, activeMembers, activeToday, activityPercent, qualified ? bonusRate : 0]
    );
    return {
      snapshot: saved.rows[0],
      nextDayBonusActive: qualified,
      explanation: { activeMembers, activeToday, activityPercent, threshold, minMembers, bonusRate }
    };
  });
}
...
async function getApplicableModifier({ userId, date = new Date() }) {
  const result = await query(
    `SELECT m.squad_id, b.bonus_date, b.bonus_rate
       FROM squad_memberships m
       JOIN squad_daily_bonus_days b ON b.squad_id = m.squad_id
      WHERE m.user_id = $1 AND m.status = 'active'
        AND b.bonus_date = ($2::date - INTERVAL '1 day')::date
        AND b.qualified = TRUE`,
    [userId, date]
  );
  ...
  return withTransaction(async client => {
    const result = await client.query(
      `INSERT INTO squad_goals
        (squad_id,title,description,target_type,target_quantity,reward_currency,reward_pool,starts_at,expires_at,status)
       VALUES ($1,$2,$3,$4,$5,'DZX',$6,$7,$8,$9) RETURNING *`,
      [squadId, title, description, targetType, target, pool, startsAt, expiresAt, status]
    );
    return result.rows[0];
  });
}
...
async function recordGoalContribution({ goalId, activityEventId, userId, quantity }) {
  ...
  return withTransaction(async client => {
    ...
    const eventResult = await client.query(
      `SELECT * FROM squad_activity_events WHERE id = $1 AND user_id = $2 AND squad_id = $3 FOR SHARE`,
      [required(activityEventId, 'activityEventId'), userId, goal.squad_id]
    );
    ...
    const contribution = await client.query(
      `INSERT INTO squad_goal_contributions
        (goal_id,user_id,activity_event_id,contribution_quantity,weight)
       VALUES ($1,$2,$3,$4,$4) RETURNING *`,
      [goalId, userId, activityEventId, amount]
    );
...
async function getGoalProgress(goalId) {
  const result = await query(
    `SELECT g.*, COALESCE(SUM(c.contribution_quantity),0) AS progress,
            COUNT(DISTINCT c.user_id)::int AS contributor_count
       FROM squad_goals g LEFT JOIN squad_goal_contributions c ON c.goal_id = g.id
      WHERE g.id = $1 GROUP BY g.id`, [required(goalId, 'goalId')]
  );
  ...
  return withTransaction(async client => {
    ...
    const contributions = await client.query(
      `SELECT user_id, SUM(weight) AS weight
         FROM squad_goal_contributions WHERE goal_id = $1 GROUP BY user_id ORDER BY user_id`, [goalId]
    );
    ...
    const distributions = contributions.rows.map(row => {
      ...
      return {
        ...
        totalWeight,
        rewardAmount: reward,
        calculation: { formula: 'reward_pool * member_weight / total_weight', reward_pool: pool, member_weight: weight, total_weight: totalWeight }
      };
    });
...
async function snapshotGoalDistribution(goalId) {
  ...
  return withTransaction(async client => {
    ...
    for (const item of calculated.distributions) {
      const result = await client.query(
        `INSERT INTO squad_goal_distributions
          (goal_id,user_id,weight,total_weight,reward_amount,calculation,idempotency_key)
         VALUES ($1,$2,$3,$4,$5,$6,$7) RETURNING *`,
        [goalId, item.userId, item.weight, item.totalWeight, item.rewardAmount, item.calculation, `squad-goal:${goalId}:user:${item.userId}`]
      );
