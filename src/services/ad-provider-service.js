...
class AdProviderRegistry {
  ...
  register(provider) {
    ...
    if (this.providers.has(provider.id))
      throw new Error(`Duplicate advertisement provider: ${provider.id}`);
    this.providers.set(provider.id, { enabled: true, ...provider });
    this.contextEnabled.set(
      provider.id,
      new Map(provider.contexts.map((context) => [context, true])),
    );
  ...
  setContextEnabled(providerId, context, enabled) {
    ...
    if (!provider.contexts.includes(context))
      throw new Error(
  ...
  listAvailable(context) {
    ...
    return [...this.providers.values()].filter(
      (provider) =>
        ...
        this.isContextEnabled(provider.id, context),
    );
  }
}
function selectNextProvider(registry, { context, previousProviderId = null }) {
  ...
  if (previousIndex < 0)
    throw new Error(
      `Unknown previous advertisement provider: ${previousProviderId}`,
    );
...
function getProviderForVerification(registry, { context, providerId }) {
  ...
  if (
    ...
    !provider.enabled ||
    !provider.contexts.includes(context) ||
    !registry.isContextEnabled(providerId, context)
  )
    throw new Error(
          ...
          () =>
            reject(
              new ProviderUnavailableError(
                `Advertisement provider ${provider.id} timed out`,
              ),
            ),
          timeoutMs,
        );
...
async function verifyWithProvider(
  ...
  const verification = await verifyProviderWithTimeout(
    provider,
    payload,
    timeoutMs,
  );
