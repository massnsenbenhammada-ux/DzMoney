...
function diggingReward(config) {
  ...
  const result = weightedChoice(weights);
  const reward = { coin_100: { coin: 100 }, dzx_1: { dzx: 1 }, dzp_1: { dzp: 1 }, jackpot: { dzx: Number(config.digging.jackpotRewardDzx) } }[result] || {};
  return { result, reward };
}
...
async function spin({ userId, idempotencyKey }) {
  ...
  return withTransaction(async client => {
    ...
    const configRow = await getConfig(client); const config = configRow.config;
...
function buildBoard(config) {
  return Array.from({ length: Number(config.digging.boardSize) }, (_, index) => ({ id: index + 1, revealed: false, result: diggingReward(config).result, reward: {} }));
}
...
function diggingRewardFromResult(result) {
  return { coin_100: { coin: 100 }, dzx_1: { dzx: 1 }, dzp_1: { dzp: 1 }, jackpot: { dzx: 10 } }[result] || {};
}

async function revealDiggingTile({ userId, sessionId, tileId }) {
  requiredId(userId, 'userId'); requiredId(sessionId, 'sessionId');
  const tileIdNumber = Number(tileId);
  if (!Number.isInteger(tileIdNumber) || tileIdNumber <= 0) throw new Error('Invalid tile');
  return withTransaction(async client => {
    ...
    const session = sessionResult.rows[0]; const board = session.board;
    const tile = board.find(item => item.id === tileIdNumber);
    ...
    const config = configResult.rows[0].config; const account = await ensureAccount(client, userId, config);
    if (account.energy_remaining < 1) throw new Error('No more digs today');
    const reward = tile.result === 'extra_axe' ? {} : tile.result === 'jackpot' ? { dzx: Number(config.digging.jackpotRewardDzx) } : diggingRewardFromResult(tile.result);
...
async function startGamingAdvertisement({ userId, game, idempotencyKey, providerRegistry }) {
  ...
  return withTransaction(async client => {
    const configRow = await getConfig(client); const account = await ensureAccount(client, userId, configRow.config);
    ...
    if (Number(count.rows[0].count) >= Number(configRow.config.dailyAdLimit)) throw new Error('Gaming daily ad limit reached');
    const provider = selectProvider(providerRegistry, { context: 'gaming' });
    const externalAdId = randomUUID();
    const event = await client.query("INSERT INTO activity_ad_events(user_id,context,external_ad_id,idempotency_key,started_at,metadata) VALUES($1,'gaming',$2,$3,$4,NOW()) RETURNING *", [userId, externalAdId, idempotencyKey, { game, provider_id: provider.id, config_version: configRow.version }]);
...
async function finalizeGamingAdvertisement({ userId, adEventId, providerReference, verificationMetadata = {} }) {
  ...
  return withTransaction(async client => {
    ...
    if (event.metadata?.gaming_reward_transaction_id) return { duplicate: true, rewarded: true, event };
    if (!event.verified) {
      ...
      if (verified.rowCount) eventResult.rows[0] = verified.rows[0];
    }
    const current = eventResult.rows[0]; const configResult = await client.query('SELECT config FROM gaming_config_versions WHERE version=$1', [current.metadata.config_version]);
    if (!configResult.rowCount) throw new Error('Gaming configuration version not found');
    const config = configResult.rows[0].config; const account = await ensureAccount(client, userId, config); const game = current.metadata.game;
    const bonus = weightedChoice(config.adBonus); const reward = bonus === 'coin_100' ? { coin: 100 } : { dzx: 1 };
    const progress = game === 'spin' ? account.spin_ad_progress + 1 : account.digging_ad_progress + 1;
