...
async function recordVerifiedActivityOnClient(client, { userId }) {
  ...
  const result = await client.query('UPDATE gaming_accounts SET spins=spins+1,axes=axes+$2,verified_activity_count=$3,updated_at=NOW() WHERE user_id=$1 RETURNING *', [userId, axe, count]);
...
async function revealDiggingTile({ userId, sessionId, tileId }) {
  ...
  return withTransaction(async client => {
    const result = await client.query("SELECT * FROM gaming_sessions WHERE id=$1 AND user_id=$2 AND status='active' FOR UPDATE", [sessionId, userId]);
    if (!result.rowCount) throw new Error('Active digging session not found');
    const session = result.rows[0], board = session.board, tile = board.find(item => item.id === tileIdNumber);
    if (!tile) throw new Error('Tile not found');
    if (tile.revealed) return { duplicate: true, tile, session };
    const cfg = await client.query('SELECT config FROM gaming_config_versions WHERE version=$1', [session.config_version]);
...
async function finalizeGamingAdvertisement({ userId, adEventId, providerReference, verificationMetadata = {} }) {
  ...
  return withTransaction(async client => {
    const found = await client.query("SELECT * FROM activity_ad_events WHERE id=$1 AND user_id=$2 AND context='gaming' FOR UPDATE", [adEventId, userId]);
    if (!found.rowCount) throw new Error('Gaming advertisement event not found');
    ...
    if (!event.verified) {
      ...
      if (verified.rowCount) found.rows[0] = verified.rows[0];
    }
    const current = found.rows[0], configResult = await client.query('SELECT config FROM gaming_config_versions WHERE version=$1', [current.metadata.config_version]);
    if (!configResult.rowCount) throw new Error('Gaming configuration version not found');
    const config = configResult.rows[0].config, account = await ensureAccount(client, userId, config), game = current.metadata.game;
    const bonus = weightedChoice(config.adBonus), reward = bonus === 'coin_100' ? { coin: 100 } : { dzx: 1 };
...
module.exports = { getGamingState, spin, startDigging, revealDiggingTile, startGamingAdvertisement, finalizeGamingAdvertisement, recordVerifiedActivityOnClient, updateGamingConfig, getConfig, validateGamingConfig };
