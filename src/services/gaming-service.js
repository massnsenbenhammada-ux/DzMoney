...
async function ensureAccount(client, userId, config) {
  ...
  await client.query(
    `UPDATE gaming_accounts SET energy_remaining=$2,energy_day=${gamingDaySql} WHERE user_id=$1 AND energy_day<>${gamingDaySql}`,
    [userId, config.digging.energy],
  );
...
async function creditGamingReward(
  client,
  userId,
  idempotencyKey,
  reward,
...
function buildBoard(config) {
  return Array.from({ length: Number(config.digging.boardSize) }, (_, i) => ({
    id: i + 1,
    revealed: false,
    result: diggingReward(config).result,
    reward: {},
  }));
}
...
async function startGamingAdvertisement({
  userId,
  game,
  idempotencyKey,
  providerRegistry,
  ...
  return withTransaction(async (client) => {
    ...
    const account = await ensureAccount(client, userId, config);
    const existing = await client.query(
      "SELECT * FROM activity_ad_events WHERE user_id=$1 AND context='gaming' AND idempotency_key=$2",
      [userId, idempotencyKey],
    );
    if (existing.rowCount)
      return {
        adEvent: existing.rows[0],
        providerId: existing.rows[0].metadata?.provider_id,
        duplicate: true,
      };
    const count = await client.query(
      `SELECT COUNT(*)::int AS count FROM activity_ad_events WHERE user_id=$1 AND context='gaming' AND metadata->>'game'=$2 AND verified=TRUE AND (completed_at + INTERVAL '1 hour')::date=${gamingDaySql}`,
      [userId, game],
    );
...
async function finalizeGamingAdvertisement({
  userId,
  adEventId,
  providerReference,
  ...
  return withTransaction(async (client) => {
      ...
      const verified = await client.query(
        ...
        [
          ...
          JSON.stringify({
            ...
            provider_verification: verificationMetadata,
          }),
        ],
      );
    ...
    await client.query(
      ...
      [
        event.id,
        JSON.stringify({
          gaming_reward_transaction_id: economy.transaction?.id || null,
          ad_bonus: reward,
...
async function getGamingState({ userId }) {
  ...
  return withTransaction(async (client) => {
    ...
    const ads = await client.query(
      `SELECT metadata->>'game' AS game,COUNT(*)::int AS count FROM activity_ad_events WHERE user_id=$1 AND context='gaming' AND verified=TRUE AND (completed_at + INTERVAL '1 hour')::date=${gamingDaySql} GROUP BY metadata->>'game'`,
      [userId],
    );
    return {
      ...
      adCounts: Object.fromEntries(
        ads.rows.map((row) => [row.game, row.count]),
      ),
      config: configRow.config,
    };
  });
...
module.exports = {
  ...
  startDigging,
  revealDiggingTile,
  startGamingAdvertisement,
  finalizeGamingAdvertisement,
  recordVerifiedActivityOnClient,
  updateGamingConfig,
  getConfig,
