...
async function startAdvertisementEvent({
  userId,
  context,
  idempotencyKey,
  externalAdId = null,
...
async function startRotatedAdvertisementEventOnClient(
  client,
  {
    userId,
    context,
  ...
  const provider = selectNextProvider(providerRegistry, {
    context,
    previousProviderId: previous.rows[0]?.provider_id || null,
  });
...
async function startPinnedAdvertisementEventOnClient(
  client,
  {
    userId,
    context,
  ...
  if (
    !provider ||
    !provider.enabled ||
    !provider.contexts.includes(context) ||
    !providerRegistry.isContextEnabled(providerId, context)
  )
    throw new Error(
...
async function markAdvertisementVerified({
  adEventId,
  providerReference,
