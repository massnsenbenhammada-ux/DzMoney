...
function publicSession(session) {
  ...
  return {
    ...session,
    board: (session.board || []).map((tile) =>
      tile.revealed ? tile : { id: tile.id, revealed: false, reward: {} },
    ),
  };
}
...
router.post(
  ...
  asyncRoute(async (req, res) => {
    ...
    const event = await query(
      "SELECT * FROM activity_ad_events WHERE id=$1 AND user_id=$2 AND context='gaming'",
      [adEventId, userId],
    );
    if (!event.rowCount)
    ...
    const result = await gaming.finalizeGamingAdvertisement({
      userId,
      adEventId,
      providerReference: verification.verification.reference,
    ...
    res.json({
      ...
      resourceGranted: result.resourceGranted || null,
