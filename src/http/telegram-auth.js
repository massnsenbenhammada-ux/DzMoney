...
async function telegramAuth(req, res, next) {
  const queryInitData =
    req.path === '/stream' ? req.query?.initData : undefined;
  const initData =
    req.get('X-Telegram-Init-Data') || req.body?.initData || queryInitData;
  ...
  if (!req.skipAccountStatusCheck) {
    try {
      const result = await query(
        'SELECT account_status FROM users WHERE telegram_user_id = $1',
        [String(verified.user.id)],
      );
      const status = result.rows[0]?.account_status || 'active';
      if (status !== 'active')
        return res.status(403).json({ error: `Account is ${status}` });
