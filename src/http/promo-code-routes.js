...
function createPromoCodeRouter({ wallet = walletService, promo = promoService, providerRegistry = providerRegistryRuntime, auth = telegramAuth } = {}) {
  ...
  async function currentUser(req) {
    return wallet.createUser({ telegramUserId: String(req.telegramUser.id), username: req.telegramUser.username || null, firstName: req.telegramUser.first_name || null, photoUrl: req.telegramUser.photo_url || null });
  }

  router.post('/redeem', asyncRoute(async (req, res) => {
    const code = req.body?.code;
    const idempotencyKey = req.body?.idempotencyKey;
    if (typeof code !== 'string' || !code.trim()) return res.status(400).json({ ok: false, error: 'Promo code is required' });
    if (typeof idempotencyKey !== 'string' || !idempotencyKey.trim()) return res.status(400).json({ ok: false, error: 'idempotencyKey is required' });
    const user = await currentUser(req);
    const result = await promo.redeemPromoCode({ userId: user.id, code, idempotencyKey, providerRegistry });
  ...
  router.get('/redemption/:id', asyncRoute(async (req, res) => {
    ...
    const user = await currentUser(req);
    res.json({ ok: true, redemption: await promo.getPromoRedemption({ userId: user.id, redemptionId: id }) });
  }));
