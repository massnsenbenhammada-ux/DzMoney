...
} = require("../services/daily-checkin-service");
const {
...
function createOnclickaPostbackRouter({ providerRegistry }) {
    ...
    try {
      ...
      if (eventResult.rowCount !== 1) {
        if (eventResult.rowCount > 1)
          return res
            ...
            .json({
              ok: false,
              error: "Multiple pending advertisement events found",
            });
        return res
          .status(404)
      ...
      if (context === "task") {
        const verified =
          await taskAdvertisementService.verifyTrustedTaskAdvertisement({
            ...
            providerPayload: {
              ...providerPayload,
              reference: event.external_ad_id,
            },
            providerRegistry,
          });
        if (!verified.adEvent)
          return res.status(202).json({ ok: true, context, verified: false });
        const finalization =
        ...
        return res.json({
          ...
          context,
          verified: true,
          duplicate: verified.duplicate || finalization.duplicate,
      ...
      if (context === "gaming") {
        ...
        return res.json({
          ...
          rewarded: finalization.rewarded,
          resourceGranted: finalization.resourceGranted || null,
          progress: finalization.progress || null,
        });
      }
      if (context === "squad") {
        ...
        return res.json({
          ...
          duplicate: verified.duplicate || finalization.duplicate,
          rewarded: finalization.rewarded === true,
          reward: finalization.reward || null,
        });
      }
      if (context === "promo") {
      ...
      if (context === "daily_checkin") {
        ...
        const reward = await finalizeDailyCheckin({
          userId: event.user_id,
          claimIdempotencyKey: event.claim_idempotency_key,
        });
        return res.json({
          ok: true,
          context,
          verified: verified.duplicate || true,
          duplicate: verified.duplicate || reward.duplicate,
      ...
      const finalization = await finalizeTaskVerification({
        attemptId: event.attempt_id,
        idempotencyKey: `task:${event.attempt_id}`,
      });
      return res.json({
        ok: true,
        context,
  ...
  router.get("/:context", (req, res, next) => {
    if (!CONTEXTS.has(req.params.context))
      return res
        .status(404)
        .json({ ok: false, error: "Unsupported advertisement context" });
