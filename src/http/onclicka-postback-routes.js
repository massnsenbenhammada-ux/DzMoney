...
function createOnclickaPostbackRouter({ providerRegistry, secret }) {
  ...
  router.get('/:context', async (req, res, next) => {
    ...
    try {
      ...
      const eventResult = await query(`SELECT a.id,a.user_id,a.context,a.verified,a.metadata,d.claim_idempotency_key,g.attempt_id,u.telegram_user_id FROM activity_ad_events a JOIN users u ON u.id=a.user_id LEFT JOIN daily_checkins d ON d.ad_event_id=a.id LEFT JOIN task_verification_gates g ON g.ad_event_id=a.id WHERE a.context=$1 AND a.verified=FALSE AND a.metadata->>'provider_id'=$2 AND u.telegram_user_id=$3 ORDER BY a.started_at DESC LIMIT 2`, [context, ONCLICKA_PROVIDER_ID, userId]);
      ...
      if (context === 'gaming') {
        const verification = await gamingService.finalizeGamingAdvertisement({ userId: event.user_id, adEventId: event.id, providerReference: `onclicka:${userId}`, verificationMetadata: { provider_id: ONCLICKA_PROVIDER_ID, confirmedByPostback: true } });
