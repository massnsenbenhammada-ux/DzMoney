...
function createOnclickaPostbackRouter({ providerRegistry }) {
  ...
  const handlePostback = async (req, res, next) => {
    ...
    try {
      ...
      const eventResult = await query(`SELECT a.id,a.user_id,a.context,a.verified,a.external_ad_id,a.metadata,d.claim_idempotency_key,g.attempt_id,u.telegram_user_id FROM activity_ad_events a JOIN users u ON u.id=a.user_id LEFT JOIN daily_checkins d ON d.ad_event_id=a.id LEFT JOIN task_verification_gates g ON g.ad_event_id=a.id WHERE a.context = ANY($1::text[]) AND a.verified=FALSE AND a.metadata->>'provider_id'=$2 AND u.telegram_user_id=$3 ORDER BY a.started_at DESC LIMIT 2`, [[...CONTEXTS], ONCLICKA_PROVIDER_ID, userId]);
      ...
      if (context === 'task') {
        const verified = await taskAdvertisementService.verifyTrustedTaskAdvertisement({ providerId: ONCLICKA_PROVIDER_ID, providerPayload: { ...providerPayload, reference: event.external_ad_id }, providerRegistry });
      ...
      if (context === 'gaming') {
        const { verification } = await verifyWithProvider(providerRegistry, { context: 'gaming', providerId: ONCLICKA_PROVIDER_ID, payload: providerPayload });
        if (!verification.verified) return res.status(202).json({ ok: true, context, verified: false });
        const finalization = await gamingService.finalizeGamingAdvertisement({ userId: event.user_id, adEventId: event.id, providerReference: verification.reference, verificationMetadata: { ...verification.metadata, confirmedByPostback: true } });
      ...
      if (context === 'squad') {
        const { verification } = await verifyWithProvider(providerRegistry, { context: 'squad', providerId: ONCLICKA_PROVIDER_ID, payload: providerPayload });
        if (!verification.verified) return res.status(202).json({ ok: true, context, verified: false });
        ...
        return res.json({ ok: true, context, verified: true, duplicate: finalization.duplicate, rewarded: finalization.rewarded === true, reward: finalization.reward || null });
      }
