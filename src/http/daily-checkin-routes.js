...
function createDailyCheckinRouter({
  providerRegistry,
  tasks = dailyTasks,
  ...
  router.get(
    ...
    asyncRoute(async (req, res) => {
      ...
      if (pending) {
        ...
        const retryable =
          Number.isFinite(pendingSince) && Date.now() - pendingSince >= 30_000;
        return res.json({
          ok: true,
          status: 'pending',
          attemptId: pending.id,
          pendingSince: new Date(pending.executed_at).toISOString(),
          retryable,
        });
      }
  ...
  router.post(
    ...
    asyncRoute(async (req, res) => {
      ...
      if (
        ...
        return res
          .status(400)
          .json({ ok: false, error: 'providerId must be a string' });
      const user = await getAuthenticatedUser(req);
      const result = await tasks.executeSystemTask({
        systemKey: DAILY_SYSTEM_TASKS.CHECK_IN,
        userId: user.id,
        idempotencyKey: body.idempotencyKey,
        metadata: { source: 'legacy_daily_checkin_compatibility' },
      });
      ...
      res.json({
        ok: true,
        claimIdempotencyKey: body.idempotencyKey,
        adEvent: {
          external_ad_id: verificationAd.adEvent?.external_ad_id || null,
        },
        providerId: verificationAd.providerId || null,
        attemptId: result.attempt.id,
...
async function getPendingAttempt(taskId, userId) {
  const result = await require('../db/pool').query(
    "SELECT id, executed_at FROM task_attempts WHERE task_id=$1 AND user_id=$2 AND status='verification_pending' ORDER BY id DESC LIMIT 1",
    [taskId, userId],
  );
...
async function getLatestCompletion(taskId, userId) {
  const result = await require('../db/pool').query(
    "SELECT GREATEST(COALESCE((SELECT MAX(verified_at) FROM task_attempts WHERE task_id=$1 AND user_id=$2 AND status='verified'), '-infinity'::timestamptz), COALESCE((SELECT last_claimed_at FROM daily_checkins WHERE user_id=$2), '-infinity'::timestamptz)) AS completed_at",
    [taskId, userId],
  );
