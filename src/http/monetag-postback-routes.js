  ...
  router.get('/', async (req, res, next) => {
    ...
    try {
      ...
      const eventResult = await query(
        `SELECT a.id,a.user_id,a.context,a.external_ad_id,a.verified,a.metadata,u.telegram_user_id,d.claim_idempotency_key,g.attempt_id
         ...
         WHERE a.context IN ('daily_checkin','verification','achievement') AND a.external_ad_id=$1`,
        [String(payload.ymid || '')]
      );
      if (eventResult.rowCount !== 1) return res.status(404).json({ ok: false, error: 'Advertisement event not found' });
      const event = eventResult.rows[0];
      const expectedContext = event.context === 'achievement' ? 'verification' : event.context;
      validateMonetagPostback(payload, expectedContext);
      ...
      if (event.context === 'achievement') {
        const result = await finalizeVerifiedAchievementAd({ event, providerRegistry, providerId: MONETAG_PROVIDER_ID, providerPayload: payload });
        if (!result.verified) return res.status(202).json({ ok: true, verified: false });
        return res.json({ ok: true, context: event.context, verified: true, duplicate: result.duplicate, rewarded: result.rewarded === true });
      }
