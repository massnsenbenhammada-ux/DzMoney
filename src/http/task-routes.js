...
function createTaskRouter({
  ...
  advertisement = taskAdvertisementService,
  ...
  const validateExecuteBody = (body) => {
    ...
    if (!Number.isInteger(body.taskId) || body.taskId <= 0)
  ...
  const validateUrlFormatBody = (body) => {
    ...
    if (Object.keys(body).some((key) => !allowed.has(key)))
      return "unknown request field";
    if (!Number.isInteger(body.attemptId) || body.attemptId <= 0)
      return "attemptId must be a positive integer";
    if (typeof body.userUrl !== "string" || body.userUrl.trim() === "")
      return "userUrl is required";
    return null;
  };
  ...
  router.get(
    ...
    asyncRoute(async (req, res) => {
      const tasksList = await tasks.listActiveTasks({
        userId: req.telegramUser.id,
      });
      res.json({
        success: true,
        tasks: tasksList.filter((task) => task.systemKey !== "squad_ads"),
      });
    }),
  );
  ...
  router.post(
    ...
    asyncRoute(async (req, res) => {
      ...
      const result = await tasks.executeTask({
        taskId,
        userId: user.id,
        idempotencyKey,
      ...
      const verificationAd = await verification.startTaskVerificationAd({
        ...
        idempotencyKey:
          result.gate?.idempotency_key || `verification:${result.attempt?.id}`,
        providerRegistry,
      });
      res.json({
        ok: true,
  ...
  router.post(
    ...
    asyncRoute(async (req, res) => {
      ...
      res.json({
        ok: true,
        adEventId: result.adEvent?.id,
        externalAdId: result.adEvent?.external_ad_id || null,
  ...
  router.post(
    ...
    asyncRoute(async (req, res) => {
      ...
      const result = await advertisement.finalizeTaskAdvertisement({
        userId: user.id,
        adEventId,
      });
      res.json({
        ...
        duplicate: result.duplicate,
        rewardIdempotencyKey: result.rewardIdempotencyKey || null,
      });
    }),
  );
  ...
  router.post(
    ...
    asyncRoute(async (req, res) => {
      ...
      const result = await tasks.recordTaskClick({
        attemptId,
        userId: user.id,
      });
  ...
  router.get(
    ...
    asyncRoute(async (req, res) => {
      ...
      const status = await verification.getTaskVerificationStatus({
        attemptId,
        userId: user.id,
      });
