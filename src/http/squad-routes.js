...
async function getSquadAdsTask() {
  const result = await query(
    "SELECT id,title,description,config FROM activity_tasks WHERE status='active' AND creator_id IS NULL AND config->>'systemKey'='squad_ads' LIMIT 1",
  );
  return result.rows[0] || null;
}
function dailyAdvertisementDateFilter() {
  return " AND (completed_at + INTERVAL '1 hour')::date=(NOW() + INTERVAL '1 hour')::date";
}
...
router.get(
  ...
  asyncRoute(async (req, res) => {
    ...
    const tierIndex = tiers.findIndex(
      (tier) =>
        memberCount >= tier.minMembers && memberCount <= tier.maxMembers,
    );
    const tier = tierIndex >= 0 ? tiers[tierIndex] : null;
    const requiredMembers = tier ? tier.maxMembers : null;
    const progressPercent = requiredMembers
      ? Math.min(100, Math.round((memberCount / requiredMembers) * 100))
      : null;
    res.json({
      ok: true,
      squad: {
        ...
        tierLevel: tierIndex >= 0 ? tierIndex : null,
        requiredMembers,
        progressPercent,
      },
    });
  ...
  asyncRoute(async (req, res) => {
    ...
    if (req.query.adEventId) {
      ...
      const reward = rewarded
        ? {
            coin: Number(metadata.reward_coin || 0),
            dzx: Number(metadata.reward_dzx || 0),
            dzp: Number(metadata.reward_dzp || 0),
          }
        : null;
      return res.json({
        ok: true,
        ...
        event: {
          ...
          verified: event.rows[0].verified === true,
          rewarded,
          reward,
          completedAt: event.rows[0].completed_at,
        },
      });
    }
    res.json({
    ...
    res
      ...
      .json({
        ...
        membership: {
          ...
          squad_id: String(result.membership.squad_id),
...
router.post(
  ...
  asyncRoute(async (req, res) => {
    ...
    const invitation = await createInvitation({
      squadId,
      inviterUserId,
      inviteeUserId: invitee.rows[0].id,
    });
    res
      .status(201)
      .json({
        ...
        invitation: {
          ...
          squad_id: String(invitation.squad_id),
          invitee_user_id: String(invitation.invitee_user_id),
        },
      });
  }),
);
    ...
    res
      ...
      .json({
        ...
        membership: {
          ...
          squad_id: String(membership.squad_id),
