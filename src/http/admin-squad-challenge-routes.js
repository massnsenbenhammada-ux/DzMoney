  ...
  router.post("/challenges", async (req, res, next) => {
    try {
      ...
      const challenge = await createWeeklyChallenge({
        ...
        rewardCurrency,
        rewardAmount,
        adminTelegramUserId: req.adminTelegramUserId,
      });
      res
        .status(201)
        .json({ ok: true, challenge: serializeChallenge(challenge) });
    } catch (error) {
      next(error);
    }
  });
...
function serializeChallenge(challenge) {
  return {
    ...
    squadId: String(challenge.squad_id),
    name: challenge.name,
    scope: challenge.scope_type,
    scopeValue: challenge.scope_value,
    rewardCurrency: challenge.reward_currency,
    rewardAmount: String(challenge.reward_amount),
    startsAt: challenge.starts_at,
    endsAt: challenge.ends_at,
    status: challenge.status,
    configSnapshot: challenge.config_snapshot,
