...
function createDailySystemTaskRouter({
  ...
  router.get(
    ...
    asyncRoute(async (req, res) => {
      ...
      if (task.config?.achievementThreshold !== undefined) {
        ...
        const claimedResult =
          await dailyTasks.assertReferralAchievementAvailable(task, user.id);
        ...
        return res.json({
          ...
          task: {
            ...
            rewardDzx: Number(task.reward_dzx),
            rewardDzp: Number(task.reward_dzp),
            available: claimedResult,
            progress: {
              completed,
              target,
              claimable: isReferralAchievementClaimable(
                qualified,
                target,
                !claimedResult,
              ),
    ...
    asyncRoute(async (req, res) => {
      ...
      if (systemKey === DAILY_SYSTEM_TASKS.VIEW_ADS) {
        ...
        return res.json({
          ...
          adsgramBlockId: result.adEvent.metadata?.adsgram_block_id || null,
  ...
  router.post(
    ...
    asyncRoute(async (req, res) => {
      ...
      await verification.getTaskVerificationStatus({
        attemptId,
        userId: user.id,
      });
  ...
  router.post(
    ...
    asyncRoute(async (req, res) => {
      ...
      const result = await adsgram.markClientStarted({
        userId: user.id,
        adEventId,
      });
      res.json({ ok: true, ...result });
  ...
  router.post(
    ...
    asyncRoute(async (req, res) => {
      ...
      const result = await adsgram.markClientCompleted({
        userId: user.id,
        adEventId,
      });
      if (result.ready) {
        const finalization = await advertisement.finalizeTaskAdvertisement({
          userId: user.id,
          adEventId,
        });
