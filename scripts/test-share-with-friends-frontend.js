...
assert.match(
  taskRoutes,
  /recordTaskClick\(\{ attemptId, userId: user\.id \}\)/,
);
assert.match(
  taskRoutes,
  /finalizeTaskVerification\(\{ attemptId, idempotencyKey: `task:\$\{attemptId\}` \}\)/,
);
...
assert.match(
  economyService,
  /ACTIVITY_REWARD_SOURCES = \['advertisement', 'task'/,
);
...
assert.match(
  appJs,
  /if \(status\.status === 'verified'\) \{ await loadMe\(\); showRewardOutcome\(status, task\);/,
);
