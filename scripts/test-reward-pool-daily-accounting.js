...
async function reward(userId, amount, source, suffix, createdAt) {
  ...
  if (createdAt) {
    await query(
      `UPDATE ledger_transactions SET created_at=$1 WHERE idempotency_key=$2`,
      [createdAt, `reward-pool-daily:${suffix}`]
    );
  }
}
...
test('Reward Pool daily accounting uses only earned activity DZP and settles idempotently', { skip: !process.env.DATABASE_URL }, async () => {
  ...
  try {
    ...
    const balanceA = await economyService.getEconomySettings();
    assert.equal(balanceA['reward_pool.daily_dzx'], '30');
    ...
    await query('DELETE FROM activity_ad_events WHERE user_id IN ($1,$2)', [userA.id, userB.id]);
    await query('DELETE FROM ledger_entries WHERE wallet_account_id IN (SELECT id FROM wallet_accounts WHERE user_id IN ($1,$2))', [userA.id, userB.id]);
    await query('DELETE FROM ledger_transactions WHERE user_id IN ($1,$2)', [userA.id, userB.id]);
