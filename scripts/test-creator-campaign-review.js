...
async function main() {
  ...
  try {
    ...
    await postEconomyTransaction({
      ...
      movements: [{ currency: "DZX", amount: 200, source: "test" }],
    });

    const campaign = await createCreatorCampaign({
      taskType: "social",
      title: "Review lifecycle contract",
    ...
    await assert.rejects(
      () => approveCreatorCampaign(taskId),
      /cannot transition from draft to active|pending_review/i,
    );
    ...
    const second = await createCreatorCampaign({
      ...
      creatorId: creator.id,
      target: 1,
      idempotencyKey: `${marker}:rejected`,
      rewardCoin: 1000,
      rewardDzx: 1,
      rewardDzp: 1,
      verificationAdSeconds: 5,
    ...
    try {
      await assert.rejects(
        () => rejectCreatorCampaign(second.task.id, creator.id),
        /pending_review/,
      );
      ...
      await pool.query("DELETE FROM activity_tasks WHERE id=$1", [
        second.task.id,
      ]);
    }
