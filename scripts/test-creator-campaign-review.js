...
async function main() {
  ...
  try {
    ...
    // Rejection is only valid while pending review and must not activate the campaign.
    const second = await createCreatorCampaign({
      taskType: 'social', title: 'Rejected campaign', creatorId: creator.id, target: 10,
    ...
    if (creator) await pool.query('DELETE FROM users WHERE id=$1', [creator.id]);
