...
async function main() {
  ...
  try {
    ...
    const second = await createCreatorCampaign({ taskType: 'social', title: 'Rejected campaign', creatorId: creator.id, target: 1, idempotencyKey: `${marker}:rejected`, rewardCoin: 1000, rewardDzx: 1, rewardDzp: 1, verificationAdSeconds: 5, config: { test: true } });
    try {
