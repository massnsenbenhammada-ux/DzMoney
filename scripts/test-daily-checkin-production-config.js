...
async function main() {
  ...
  const result = await pool.query(
    `SELECT id FROM activity_tasks WHERE task_type='daily' AND config->>'systemKey'='daily_check_in' AND status='active' ORDER BY id`,
  );
  assert.strictEqual(
    result.rowCount,
    1,
  ...
  await withTransaction(async (client) => {
    ...
    await client.query(
      `UPDATE activity_tasks SET config=config-'dailyMode' WHERE id=$1`,
      [taskId],
    );
