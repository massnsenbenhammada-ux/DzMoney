        ...
        transactions: [
          {
            ...
            in_msg: { destination: RAW_MAINNET, value, bounced: false },
          },
        ],
      };
...
async function main() {
  ...
  try {
    const settings = await query(
      `SELECT key,value FROM admin_settings WHERE key IN ('deposit.daily_limit_ton','deposit.pending_timeout_hours','deposit.ton.active_network','deposit.ton.mainnet_address')`,
    );
    ...
    const fractional = await processDeposit({
      idempotencyKey: `${marker}:fractional`,
      userId: user.id,
      txHash: validHash,
      tonAmount: 0.1,
      confirmationCount: 0,
    ...
    const duplicate = await confirmDeposit({
      idempotencyKey: `${marker}:fractional`,
    });
    ...
    await query(
      `UPDATE deposits SET created_at=NOW()-INTERVAL '25 hours',updated_at=NOW()-INTERVAL '25 hours' WHERE idempotency_key=$1`,
      [staleKey],
    );
    const staleConfirmation = await confirmDeposit({
      idempotencyKey: staleKey,
    });
    ...
    await query(
      `DELETE FROM wallet_accounts WHERE user_id=$1 AND currency='DZX'`,
      [rollbackUser.id],
    );
    await assert.rejects(
      () => confirmDeposit({ idempotencyKey: rollbackKey }),
