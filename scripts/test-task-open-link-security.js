...
const fakeClient = { query: async sql => {
  if (sql.includes('SELECT a.*,u.telegram_user_id')) {
    return { rowCount: 1, rows: [{ id: 11, status: 'verification_pending', user_id: 42, telegram_user_id: 123, task_type: 'web', reward_coin: '1000', reward_dzx: '1', reward_dzp: '1', config: { completion: { mode: 'open_link', url: 'https://example.test/task' } }, gate_id: 12, gate_status: 'ad_completed' }] };
  }
...
async function run() {
  ...
  assert.strictEqual(transactionStarted, false, 'untrusted open-link completion must not reach reward transaction');
  console.log('Task open-link security boundary: PASS');
}

run().catch(error => { console.error('Task open-link security boundary: FAIL'); console.error(error); process.exitCode = 1; });
