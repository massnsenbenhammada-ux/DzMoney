...
async function run() {
  ...
  require.cache[dailyTasksPath].exports = {
    ...
    executeSystemTask: async args => {
      calls.push(args);
      return { attempt: { id: 42 }, gate: { id: 9, idempotency_key: 'gate-1', status: 'pending' }, duplicate: false };
    }
  };
  ...
  assert.strictEqual((await request('POST', '/api/daily-tasks/execute', { systemKey: 'check_for_update', idempotencyKey: 'check-1' })).status, 401);
  const auth = buildInitData(123);
  const result = await request('POST', '/api/daily-tasks/execute', { systemKey: 'check_for_update', idempotencyKey: 'check-1' }, auth);
...
run().catch(error => { console.error('Check for Update HTTP boundary: FAIL'); console.error(error); process.exit(1); });
