...
const attemptRow = {
  ...
  reward_dzp: '1',
  config: {
    campaignUrl: 'https://example.test/task',
...
const fakeClient = {
  query: async (sql) => {
    if (sql.includes('SELECT a.*,u.telegram_user_id'))
      return { rowCount: 1, rows: [attemptRow] };
    if (sql.includes('SELECT metadata FROM task_attempts'))
      return { rowCount: 1, rows: [{ metadata: {} }] };
...
require.cache[dbPath] = {
  exports: {
    ...
    withTransaction: async (work) => work(fakeClient),
  },
};
require.cache[economyPath] = {
  exports: {
    creditActivityRewardOnClient: async () => {
      rewardReached = true;
      throw new Error('reward must not be reached');
    },
  },
};
...
require.cache[configPath] = {
  exports: { resolveVerificationConfig: (input) => input.config },
};
...
async function run() {
  ...
  assert.strictEqual(
    rewardReached,
    false,
    'missing click proof must never reach the Economy reward path',
  );

  attemptRow.status = 'expired';
  const expired = await finalizeTaskVerification({
    attemptId: 11,
    idempotencyKey: 'task:11',
  });
  ...
  assert.strictEqual(
    rewardReached,
    false,
    'expired attempts must never reach the Economy reward path',
  );
...
run().catch((error) => {
  console.error('Task verification security boundary: FAIL');
  console.error(error);
  process.exitCode = 1;
});
