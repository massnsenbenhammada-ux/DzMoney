...
function requireEnv(name) {
  ...
  if (!value)
    throw new Error(
      `${name} is required for the real Telegram integration test`,
    );
  return value;
}
...
async function main() {
  ...
  try {
    ...
    assert.strictEqual(
      rewarded.length,
      1,
      'Concurrent real Telegram claims must create one reward',
    );
    assert.strictEqual(
      duplicates.length,
      1,
      'Concurrent real Telegram claim must produce one duplicate result',
    );
    ...
    assert.strictEqual(
      adEvents.rowCount,
      0,
      'Check for Update must not create an advertisement event',
    );
    ...
    console.log(
      `Real Telegram Check for Update: PASS (${CHANNEL}, user ${telegramUserId})`,
    );
  } finally {
    await pool.query(
