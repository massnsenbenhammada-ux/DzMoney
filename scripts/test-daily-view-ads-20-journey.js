...
async function assertFinalInvariants(userId, taskId) {
  ...
  const progress = await getAdvertisementProgress({ id: taskId, config: { systemKey: 'view_ads', advertisementTarget: 20 } }, userId);
  assert.deepStrictEqual(progress, { completed: 20, target: 20, available: false });
}
...
async function main() {
  ...
  let task;
  try {
    task = await getSystemTask('view_ads');
    ...
    assert.strictEqual(Number(task.reward_coin), 1000);
    assert.strictEqual(Number(task.reward_dzx), 1);
    assert.strictEqual(Number(task.reward_dzp), 1);
    ...
    for (let index = 1; index <= 20; index += 1) {
      ...
      if (index < 20) {
        const next = await getAdvertisementProgress(task, userId);
        assert.strictEqual(next.available, true);
        assert.strictEqual(next.completed, index);
      }
    }

    await assertFinalInvariants(userId, task.id);
    await assert.rejects(() => startTaskAdvertisement({ userId, taskId: task.id, idempotencyKey: `phase14-view-ads-${userId}-21`, providerRegistry: registry }), /Daily advertisement target is already complete|Task advertisement/);
