...
async function main() {
  ...
  let adEventId;
  try {
    const user = await pool.query(`INSERT INTO users (telegram_user_id, username, first_name) VALUES ($1,$2,$3) RETURNING id`, [telegramUserId, `squad_ads_${marker}`, 'Squad Ads Integration']);
    ...
    await withTransaction(async client => {
      ...
      const task = await client.query(`INSERT INTO activity_tasks (task_type,title,reward_coin,reward_dzx,reward_dzp,status,config) VALUES ('daily','Squad Ads integration test',1000,1,1,'active','{"systemKey":"squad_ads","advertisementTarget":10,"advertisementContext":"squad","dailyMode":"advertisement"}') RETURNING id`);
      taskId = task.rows[0].id;
      const event = await client.query(`INSERT INTO activity_ad_events (user_id,context,external_ad_id,idempotency_key,metadata) VALUES ($1,'squad',$2,$3,$4) RETURNING id`, [userId, `adsgram-integration-${marker}`, `squad-ads-integration:${marker}`, { task_id: taskId, provider_id: 'adsgram', adsgram_block_id: '44442', provider_state: { client_started: false, client_completed: false, provider_confirmed: false } }]);
      adEventId = event.rows[0].id;
    });

    await assert.rejects(() => markClientCompleted({ userId, adEventId }), /AdsGram advertisement has not started/);
    await assert.rejects(() => markProviderConfirmed({ userTelegramId: telegramUserId, providerReference: `provider:${marker}` }), /No started AdsGram advertisement matches/);
    ...
    const balances = await pool.query('SELECT currency,balance FROM wallet_accounts WHERE user_id=$1 ORDER BY currency', [userId]);
    ...
    const ledger = await pool.query(`SELECT COUNT(*)::int AS count FROM ledger_transactions WHERE user_id=$1 AND type='ACTIVITY_REWARD'`, [userId]);
...
main().catch(error => { console.error('Squad Ads integration invariants: FAIL'); console.error(error); process.exit(1); });
