...
async function main() {
  ...
  try {
    ...
    const ownerProvider = await markProviderConfirmed({ userTelegramId: owner.telegramUserId, providerReference: `provider:${marker}:owner` });
    ...
    const balances = await pool.query('SELECT currency,balance FROM wallet_accounts WHERE user_id=$1 ORDER BY currency', [owner.id]);
    ...
    await withTransaction(async client => {
      for (const user of users) {
        ...
        await client.query('DELETE FROM activity_ad_events WHERE user_id=$1', [user.id]);
        await client.query('DELETE FROM wallet_accounts WHERE user_id=$1', [user.id]);
        await client.query('DELETE FROM users WHERE id=$1', [user.id]);
      }
      for (const taskId of taskIds) await client.query('DELETE FROM activity_tasks WHERE id=$1', [taskId]);
    });
    await pool.end();
  }
}

main().catch(error => { console.error('Squad Ads integration edge cases: FAIL'); console.error(error); process.exit(1); });
