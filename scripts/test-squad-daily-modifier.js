    ...
    try {
      ...
      for (let index = 0; index < 3; index += 1) {
        const user = await walletService.createUser({
          telegramUserId: `89${suffix}${index}`,
          username: `squad_mod_${suffix}_${index}`,
        });
        users.push(user);
        ids.push(user.id);
      }
      ...
      await query(
        "INSERT INTO squad_memberships (squad_id,user_id,status) VALUES ($1,$2,'active'),($1,$3,'active'),($1,$4,'active')",
        [squadId, users[0].id, users[1].id, users[2].id],
      );
      ...
      await query(
        `INSERT INTO activity_ad_events(user_id,context,external_ad_id,idempotency_key,started_at,completed_at,verified,metadata) VALUES($1,'task',$2,$3,NOW(),NOW(),TRUE,$4),($5,'task',$6,$7,NOW(),NOW(),TRUE,$8)`,
        [
          users[1].id,
          `verified-a-${suffix}`,
          `verified-a-${suffix}`,
      ...
      const applied = await withTransaction((client) =>
        getApplicableSquadModifierOnClient(client, {
          userId: users[1].id,
          day: applicationDay,
        }),
      );
      ...
      const notContributor = await withTransaction((client) =>
        getApplicableSquadModifierOnClient(client, {
          userId: users[0].id,
          day: applicationDay,
        }),
