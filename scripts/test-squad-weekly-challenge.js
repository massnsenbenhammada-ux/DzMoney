...
const {
  ...
  distributeReward,
} = require("../src/services/squad-weekly-challenge-service");
const root = path.join(__dirname, "..");

test("Weekly Challenge exposes only the locked scopes", () => {
  assert.deepStrictEqual(VALID_SCOPES, [
    "ALL TASKS",
    "Type Tasks",
    "Verified Ad",
    "Verified Task",
...
test("scope matching uses existing activity reward metadata", () => {
  assert.strictEqual(
    matchesChallengeScope(
      { source: "task", activityType: "social" },
      "ALL TASKS",
      null,
    ),
    true,
  ...
  assert.strictEqual(
    matchesChallengeScope(
      { source: "advertisement", activityContext: "task" },
      "Verified Ad",
      null,
    ),
    true,
  ...
  assert.strictEqual(
    matchesChallengeScope(
      { source: "advertisement", activityContext: "squad" },
      "Verified Squad AdView",
      null,
    ),
    true,
  ...
  assert.strictEqual(
    matchesChallengeScope(
      { source: "advertisement", activityContext: "verification" },
      "All Activity Verified",
      null,
    ),
    true,
...
test("reward distribution is proportional, deterministic and sums to the configured reward", () => {
  const result = distributeReward("100", [
    { userId: "2", contribution: "2" },
    { userId: "1", contribution: "1" },
  ]);
  assert.deepStrictEqual(
    result.map((item) => item.rewardAmount),
    ["66.666666667", "33.333333333"],
  );
  assert.strictEqual(
    result.reduce((sum, item) => sum + BigInt(item.rewardScaled), 0n),
    100000000000n,
  );
});
...
test("reward distribution uses the canonical Economy proportional rounding primitive", () => {
  const economy = fs.readFileSync(
    path.join(root, "src/services/economy-service.js"),
    "utf8",
  );
  const squad = fs.readFileSync(
    path.join(root, "src/services/squad-weekly-challenge-service.js"),
    "utf8",
  );
  assert.match(
    economy,
  ...
  assert.match(
    squad,
    /multiplyRatioScaled\(remainingReward, item\.contribution, remainingContribution\)/,
  );
  assert.doesNotMatch(
    squad,
    /largest fractional remainder|remainder.*sort|function roundRatio/,
  );
});
...
test("weekly challenge persistence uses a snapshot and seven-day window", () => {
  const migration = fs.readFileSync(
    path.join(root, "migrations/036_squad_weekly_challenges.sql"),
    "utf8",
  );
  ...
  assert.match(
    migration,
    /reward_transaction_id BIGINT REFERENCES ledger_transactions/,
  );
});
...
test("existing Economy boundary receives activity scope metadata", () => {
  ...
  const verification = fs.readFileSync(
    path.join(root, "src/services/task-verification-service.js"),
    "utf8",
  );
  const advertisement = fs.readFileSync(
...
test("admin challenge API is protected by existing admin authentication", () => {
  const routes = fs.readFileSync(
    path.join(root, "src/http/admin-squad-challenge-routes.js"),
    "utf8",
  );
