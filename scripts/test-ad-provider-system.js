...
function testContextSelection() {
  ...
  assert.strictEqual(
    selectNextProvider(registry, { context: 'verification', previousProviderId: 'first' }).id,
    'second',
  );
  assert.strictEqual(
    selectNextProvider(registry, { context: 'verification', previousProviderId: 'second' }).id,
    'first',
  );
...
function testSquadRotationOrder() {
  ...
  assert.strictEqual(
    selectNextProvider(registry, { context: 'squad', previousProviderId: 'monetag' }).id,
    'adsgram',
  );
  assert.strictEqual(
    selectNextProvider(registry, { context: 'squad', previousProviderId: 'adsgram' }).id,
    'onclicka',
  );
  assert.strictEqual(
    selectNextProvider(registry, { context: 'squad', previousProviderId: 'onclicka' }).id,
    'monetag',
  );
...
function testProviderValidation() {
  ...
  assert.throws(
    () =>
      new AdProviderRegistry([provider('no-verify', ['verification'], { verifyCompletion: null })]),
    /verifyCompletion/,
  );
}
...
function testDisabledProviderIsSkipped() {
  ...
  assert.strictEqual(
    selectNextProvider(registry, { context: 'verification', previousProviderId: 'third' }).id,
    'first',
  );
}
...
async function testVerificationUsesRecordedProviderOnly() {
  ...
  assert.strictEqual(
    getProviderForVerification(registry, { context: 'verification', providerId: 'second' }).id,
    'second',
  );
}
...
async function testInvalidProviderResultFailsClosed() {
  const registry = new AdProviderRegistry([
    provider('malformed', ['verification'], { verifyCompletion: async () => ({ verified: true }) }),
  ]);
  await assert.rejects(
    () =>
