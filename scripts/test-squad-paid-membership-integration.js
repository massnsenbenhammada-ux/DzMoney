    ...
    try {
      ...
      for (let index = 0; index < 7; index += 1) {
        const user = await walletService.createUser({
          telegramUserId: `9${suffix}${index + 1}`,
          username: `squad_paid_${suffix}_${index}`,
        });
        users.push(user);
        ids.push(user.id);
      }
      ...
      await query(
        "INSERT INTO squad_memberships (squad_id,user_id,status) VALUES ($1,$2,'active'),($1,$3,'active'),($1,$4,'active'),($5,$6,'active'),($5,$7,'active')",
        [
          squadIds[0],
          users[0].id,
          users[2].id,
      ...
      await query(
        "UPDATE wallet_accounts SET balance = 500 WHERE user_id = $1 AND currency = 'DZP'",
        [users[5].id],
      );

      const first = await purchasePaidMembership({
        userId: users[5].id,
        maxMembers: 10,
        idempotencyKey: `paid-${suffix}`,
      });
      ...
      const balance = await query(
        "SELECT balance FROM wallet_accounts WHERE user_id = $1 AND currency = 'DZP'",
        [users[5].id],
      );
      assert.equal(Number(balance.rows[0].balance), 400);

      const duplicate = await purchasePaidMembership({
        userId: users[5].id,
        maxMembers: 10,
        idempotencyKey: `paid-${suffix}`,
      });
      ...
      const memberships = await query(
        "SELECT COUNT(*)::int AS count FROM squad_memberships WHERE user_id = $1 AND status <> 'cancelled'",
        [users[5].id],
      );
