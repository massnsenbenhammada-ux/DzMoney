...
async function createInactiveMembership(userId) {
  ...
  const membership = await pool.query(
    "INSERT INTO squad_memberships (squad_id,user_id,status) VALUES ($1,$2,'inactive') RETURNING id",
...
async function main() {
  ...
  try {
    ...
    await assert.rejects(
      ...
      /Daily Check-in advertisement must be verified first/,
    );
    await assert.rejects(
      () =>
        verifyDailyCheckinAd({
    ...
    let cooldownError;
    try {
      await startDailyCheckinClaim({
        userId,
        idempotencyKey: `daily-${Date.now()}-second`,
        providerRegistry: registry,
      });
    } catch (error) {
      cooldownError = error;
