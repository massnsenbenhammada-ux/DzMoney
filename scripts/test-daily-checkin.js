...
const provider = {
  ...
  async verifyCompletion(payload) {
    return payload?.accepted === true
      ? { verified: true, reference: payload.reference || 'daily-test-ref', metadata: {} }
      : { verified: false };
  }
};
...
async function createUser() {
  ...
  await withTransaction(async client => {
    ...
    await client.query(`INSERT INTO admin_settings(key,value) VALUES ('activity.daily_checkin_cooldown_hours','24'::jsonb),('activity.default_reward_coin','1000'::jsonb),('activity.default_reward_dzx','1'::jsonb),('activity.default_reward_dzp','1'::jsonb) ON CONFLICT (key) DO NOTHING`);
  });
  return userId;
}
