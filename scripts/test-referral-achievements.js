...
const INVITE_REWARDS = Object.freeze({
  ...
  [DAILY_SYSTEM_TASKS.INVITE_100_FRIENDS]: {
    coin: 1000000,
    dzx: 1000,
    dzp: 100,
  },
...
async function testConfiguredInviteRewards() {
  ...
  const result = await query(
    `SELECT config->>'systemKey' AS system_key, reward_coin, reward_dzx, reward_dzp, config->>'dailyPolicy' AS daily_policy FROM activity_tasks WHERE task_type='daily' AND config->>'systemKey' = ANY($1::text[])`,
    [keys],
  );
  assert.strictEqual(
    result.rowCount,
  ...
  for (const row of result.rows) {
    ...
    assert.deepStrictEqual(
      ...
      expected,
      `Unexpected reward contract for ${row.system_key}`,
    );
    assert.strictEqual(
      row.daily_policy,
      'permanent',
  ...
  try {
    for (const threshold of Object.values(REFERRAL_ACHIEVEMENT_THRESHOLDS)) {
      ...
      const verifier = resolveTrustedTaskVerifier({
        config: { achievementThreshold: threshold },
        userId: 123,
        telegramUserId: 456,
      });
      assert.strictEqual(
        await verifier({}),
        true,
        `Invite ${threshold} must verify at its threshold`,
      );

      referralService.getQualifiedReferralCount = async () => threshold - 1;
      const belowVerifier = resolveTrustedTaskVerifier({
        config: { achievementThreshold: threshold },
        userId: 123,
        telegramUserId: 456,
      });
      assert.strictEqual(
        await belowVerifier({}),
        false,
        `Invite ${threshold} must fail below the threshold`,
      );
    }
  } finally {
