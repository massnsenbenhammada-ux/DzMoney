...
async function main() {
  ...
  try {
    ...
    await assert.rejects(
      () =>
        verifyTrustedTaskAdvertisement({
          ...
          providerPayload: { accepted: true, userId: userA.telegramUserId },
          providerRegistry: registry,
        }),
    ...
    await assert.rejects(
      () =>
        verifyTrustedTaskAdvertisement({
          providerId: mismatchedIdentityProvider.id,
          providerPayload: {
            accepted: true,
            reference,
            userId: userA.telegramUserId,
    ...
    const replay = await verifyTrustedTaskAdvertisement({
      providerId: provider.id,
      providerPayload: {
        accepted: true,
        reference,
    ...
    const secondFinalize = await finalizeTaskAdvertisement({
      userId: userA.userId,
      adEventId: startedA.adEvent.id,
    });
