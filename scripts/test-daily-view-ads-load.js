...
async function main() {
  if (!process.env.TEST_TELEGRAM_USER_ID) throw new Error('TEST_TELEGRAM_USER_ID is required');
  const user = await walletService.createUser({
    telegramUserId: `${process.env.TEST_TELEGRAM_USER_ID}${Date.now()}`.slice(0, 15),
  ...
  try {
    ...
    const results = await Promise.all(Array.from({ length: CONCURRENCY }, (_, index) => (async () => {
      ...
      return { result, elapsedMs: Number(process.hrtime.bigint() - requestStartedAt) / 1e6 };
    })()));
    const elapsedMs = Number(process.hrtime.bigint() - startedAt) / 1e6;
    assert.strictEqual(results.length, CONCURRENCY);
    assert(results.every(({ result }) => result.providerId === 'monetag'));
    const samples = results.map(({ elapsedMs: duration }) => duration).sort((a, b) => a - b);
    const p95 = samples[Math.min(samples.length - 1, Math.ceil(samples.length * 0.95) - 1)];
    assert(p95 <= P95_LIMIT_MS, `Daily View advertisement start p95 ${p95.toFixed(2)}ms exceeds ${P95_LIMIT_MS}ms`);
    console.log(`Phase 14 Daily View Ads load: PASS (${CONCURRENCY} concurrent starts, p95=${p95.toFixed(2)}ms, total=${elapsedMs.toFixed(2)}ms)`);
  } finally {
