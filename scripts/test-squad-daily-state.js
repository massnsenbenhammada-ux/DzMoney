    ...
    try {
      ...
      for (let index = 0; index < 5; index += 1) {
        const user = await walletService.createUser({
          telegramUserId: `8${suffix}${index + 1}`,
          username: `squad_daily_${suffix}_${index}`,
        });
        users.push(user);
        ids.push(user.id);
      }
      ...
      await query(
        "INSERT INTO squad_memberships (squad_id,user_id,status) VALUES ($1,$2,'active'),($1,$3,'active'),($1,$4,'inactive')",
