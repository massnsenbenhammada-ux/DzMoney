...
async function main() {
  ...
  const referrer = await walletService.createUser({
    telegramUserId: `9${suffix}01`,
  });
  const referred = await walletService.createUser({
    telegramUserId: `9${suffix}02`,
  });
  const other = await walletService.createUser({
    telegramUserId: `9${suffix}03`,
  });
  ...
  assert.strictEqual(
    Number(first.attribution.referrer_user_id),
    Number(referrer.id),
  );
  assert.strictEqual(
