      ...
      const users = await query(
       ...
       RETURNING id, telegram_user_id`,
        [
          ...
          `squad_owner_${suffix}`,
          inviteeTelegramId,
          `squad_invitee_${suffix}`,
        ],
      );
      ownerId = users.rows.find(
        (row) => String(row.telegram_user_id) === ownerTelegramId,
      ).id;
      inviteeId = users.rows.find(
        (row) => String(row.telegram_user_id) === inviteeTelegramId,
      ).id;
