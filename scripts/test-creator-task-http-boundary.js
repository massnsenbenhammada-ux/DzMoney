...
async function run() {
  const {
    createCreatorTaskRouter,
  } = require("../src/http/creator-task-routes");
  const calls = [];
  const walletCalls = [];
  const wallet = {
  ...
  const tasks = {
    getCreatorCampaignContract: async (...args) => {
      assert.strictEqual(
        args.length,
        0,
        "creator campaign contract must not receive taskType as queryFn",
      );
      return { priceDZX: 10, verificationAdSeconds: 5 };
    },
    getCreatorActivityRewards: async () => ({
      rewardCoin: 1000,
      rewardDZX: 1,
      rewardDZP: 1,
    }),
    createCreatorCampaign: async (args) => {
      calls.push({ create: args });
      return {
        task: { id: 7, task_type: args.taskType, config: args.config },
        appliedPriceDZX: 10,
        campaignCostDZX: Number(args.target) * 10,
        duplicate: false,
      };
    },
    submitCreatorCampaignForReview: async (taskId, creatorId) => {
      calls.push({ submit: { taskId, creatorId } });
      return { id: 7, status: "pending_review", creator_id: creatorId };
    },
  };
  ...
  const request = (method, path, body, initData) =>
  ...
  assert.strictEqual(
    (await request("GET", "/api/creator/tasks/contracts/special")).status,
    401,
  );
  const auth = buildInitData(123);

  const specialContract = await request(
    "GET",
    "/api/creator/tasks/contracts/special",
    null,
    auth,
  );
  assert.strictEqual(specialContract.status, 400);

  const socialContract = await request(
    "GET",
    "/api/creator/tasks/contracts/social",
    null,
    auth,
  );
  assert.deepStrictEqual(socialContract.body.verificationMethods, [
    "click_proof",
    "bot_api",
  ]);
  ...
  assert.strictEqual(
    socialContract.body.campaignPricing.priceDZXPerExecution,
    10,
  );
  ...
  assert.strictEqual(
    calls[0].create.config.campaignUrl,
    "https://example.test/campaign",
  );
  assert.strictEqual(
    calls[0].create.config.verification.provider,
    "telegram_channel",
  );
  assert.strictEqual(calls[0].create.config.verification.method, "bot_api");
  assert.strictEqual(
    calls[0].create.config.verification.event,
    "channel_membership",
  );
  assert.deepStrictEqual(calls[0].create.config.verification.requirements, {
  ...
  const clickProof = await request(
    "POST",
    "/api/creator/tasks",
    {
      taskType: "social",
      title: "Click proof campaign",
      target: 1245,
      idempotencyKey: "creator-task-2",
      config: {
  ...
  const missingUrl = await request(
    "POST",
    "/api/creator/tasks",
    {
      taskType: "social",
  ...
  const legacyCompletion = await request(
    ...
    "/api/creator/tasks",
    {
      ...
      target: 1000,
      idempotencyKey: "creator-task-5",
      config: {
        completion: { mode: "open_link", url: "https://example.test" },
        verification: { method: "click_proof" },
        campaignUrl: "https://example.test",
      },
    },
  ...
  const invalidType = await request(
    "GET",
    "/api/creator/tasks/contracts/daily",
    null,
    auth,
  );
