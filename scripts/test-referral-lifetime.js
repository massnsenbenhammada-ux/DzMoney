...
async function qualifiedReferral(suffix) {
  const referrer = await walletService.createUser({
    telegramUserId: `7${suffix}01`,
  });
  const referred = await walletService.createUser({
    telegramUserId: `7${suffix}02`,
  });
  ...
  const task = await pool.query(
    "INSERT INTO activity_tasks(task_type,title,reward_coin) VALUES('game','Referral lifetime test',1) RETURNING id",
  );
...
async function main() {
  ...
  const pendingReferrer = await walletService.createUser({
    telegramUserId: `6${suffix}01`,
  });
  const pendingReferred = await walletService.createUser({
    telegramUserId: `6${suffix}02`,
  });
  await referralService.createAttribution({
    referrerUserId: pendingReferrer.id,
