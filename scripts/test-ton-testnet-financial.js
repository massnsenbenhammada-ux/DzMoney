...
async function main() {
  ...
  try {
    ...
    await withTransaction(async (client) => {
      ...
      await client.query(
        "INSERT INTO admin_settings(key,value) VALUES ('deposit.required_confirmations','1'::jsonb) ON CONFLICT (key) DO UPDATE SET value=EXCLUDED.value",
      );
    });
    ...
    const ledger = await pool.query(
      "SELECT COUNT(*)::int AS count FROM ledger_transactions WHERE user_id=$1 AND type='DEPOSIT'",
      [userId],
    );
    ...
    console.log(
      `TON testnet financial gate: PASS (deposit=${depositId}, amount=${tonAmount} TON)`,
    );
  } finally {
    await withTransaction(async (client) => {
      if (userId) {
