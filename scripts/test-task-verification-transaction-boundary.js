...
const fakeClient = {
  query: async (sql) => {
    ...
    if (sql.includes("UPDATE task_attempts SET status='verified'"))
      return { rowCount: 1, rows: [] };
    if (sql.includes("UPDATE task_verification_gates SET status='verified'"))
      return { rowCount: 1, rows: [] };
    return { rowCount: 1, rows: [] };
  },
};
...
require.cache[referralPath] = {
  exports: {
    creditReferralLifetimeOnClient: async () => ({ duplicate: false }),
  },
};
require.cache[adEventPath] = {
  exports: { markAdvertisementVerified: async () => ({ duplicate: false }) },
};
...
require.cache[telegramPath] = {
  exports: { isTelegramChannelMember: async () => true },
};
