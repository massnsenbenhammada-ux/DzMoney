...
async function run() {
  const configured = await getCreatorCampaignContract(
    queryWith([
  ...
  await assert.rejects(
    () =>
      getCreatorCampaignContract(
        queryWith([{ key: 'verification_ad_seconds', value: '30' }]),
      ),
  ...
  await assert.rejects(
    () =>
      getCreatorCampaignContract(
        queryWith([
          { key: 'task.campaign_price_dzx_per_execution', value: '0' },
        ]),
      ),
  ...
  await assert.rejects(
    () =>
      getCreatorCampaignContract(
        queryWith([
          {
            key: 'task.campaign_price_dzx_per_execution',
            value: 'not-a-number',
          },
        ]),
      ),
