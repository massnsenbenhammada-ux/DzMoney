...
async function migrate() {
  ...
  try {
    ...
    // The existing Admin Panel already edits the legacy `settings` table.
    // Mirror the three DZP controls there so the values are editable without
    // introducing a second Admin UI.
    await client.query(`
    ...
    await client.query(`
      ...
      -- Bring the DZP source-of-truth in sync with any existing Admin values.
      INSERT INTO dzp_settings(key,value,updated_at)
      SELECT 'referral_dzp_reward',GREATEST(0,value::numeric),NOW() FROM settings WHERE key='dzp_referral_reward'
      ON CONFLICT(key) DO UPDATE SET value=EXCLUDED.value,updated_at=NOW();
      INSERT INTO dzp_settings(key,value,updated_at)
      SELECT 'default_activity_dzp',GREATEST(0,value::numeric),NOW() FROM settings WHERE key='dzp_default_activity'
      ON CONFLICT(key) DO UPDATE SET value=EXCLUDED.value,updated_at=NOW();
      INSERT INTO dzp_settings(key,value,updated_at)
      SELECT 'ad_dzp_reward',GREATEST(0,value::numeric),NOW() FROM settings WHERE key='dzp_ad_reward'
      ON CONFLICT(key) DO UPDATE SET value=EXCLUDED.value,updated_at=NOW();
    `);
    ...
    await client.query(`
      ...
      BEGIN
        ...
        SELECT COALESCE(value,0) INTO dzp_amount FROM dzp_settings WHERE key='ad_dzp_reward' LIMIT 1;
        ...
        VALUES(NEW.user_id,'AD_VIEW',NEW.id::text,dzp_amount,jsonb_build_object('provider',NEW.provider,'task_id',NEW.task_id))
        ...
        IF FOUND THEN
          ...
          VALUES(NEW.user_id,'DZP','CREDIT',dzp_amount,'available','AD_VIEW',NEW.id::text,
                 jsonb_build_object('provider',NEW.provider,'task_id',NEW.task_id),COALESCE(NEW.confirmed_at,NEW.created_at));
        END IF;
        RETURN NEW;
      END;
      $$ LANGUAGE plpgsql;
