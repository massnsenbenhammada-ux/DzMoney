...
async function main() {
  ...
  try {
    ...
    // Fund the creator so the campaign can debit 90 DZX (10 executions × 9 DZX).
    ...
    const ledger = await pool.query(
      `SELECT lt.transaction_type, lt.metadata, le.amount, le.source
       FROM ledger_entries le
       JOIN ledger_transactions lt ON lt.id=le.transaction_id
       WHERE lt.user_id=$1 AND le.source IN ('creator_campaign','creator_campaign_tax','creator_campaign_refund')
       ORDER BY le.id`,
      [user.id]
    );

    const debit = ledger.rows.find(row => row.source === 'creator_campaign' && Number(row.amount) === -90);
    const tax = ledger.rows.find(row => row.source === 'creator_campaign_tax');
    const refund = ledger.rows.find(row => row.source === 'creator_campaign_refund');

    assert.ok(debit);
    assert.ok(tax);
    assert.ok(refund);
    assert.equal(Number(tax.amount), -9);
    assert.equal(Number(refund.amount), 81);
    assert.equal(Number(tax.metadata.tax_percent), 10);
    assert.equal(Number(tax.metadata.tax_dzx), 9);
    assert.equal(Number(refund.metadata.refund_dzx), 81);
    assert.equal(Number(refund.metadata.campaign_cost_dzx), 90);

    // Repeating rejection must be idempotent and must not refund twice.
