...
const route = fs.readFileSync(path.join(root, 'src/http/admin-economy-routes.js'), 'utf8');
const service = fs.readFileSync(path.join(root, 'src/services/admin-settings-service.js'), 'utf8');
...
test('test:all includes admin economy contract', () => {
  assert.equal(packageJson.scripts['test:admin-economy'], 'node ./scripts/test-admin-economy.js');
