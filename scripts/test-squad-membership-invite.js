...
const migration = fs.readFileSync(
  path.join(root, 'migrations/032_squad_membership_invites.sql'),
  'utf8',
);
const membership = fs.readFileSync(
  path.join(root, 'src/services/squad-membership-service.js'),
  'utf8',
);
const taskVerification = fs.readFileSync(
...
test('membership migration supports inactive invitation membership without a second membership table', () => {
  assert.match(
    migration,
    /status IN \('inactive', 'active', 'suspended', 'cancelled'\)/,
  );
  assert.match(migration, /CREATE TABLE IF NOT EXISTS squad_invitations/);
  assert.match(
    migration,
    /UNIQUE INDEX IF NOT EXISTS uq_squad_pending_invitation/,
  );
...
test('verified task completion activates membership inside the existing economy transaction', () => {
  assert.match(
    taskVerification,
    /activateOnVerifiedActivity\(client, row\.user_id\)/,
  );
  const rewardIndex = taskVerification.indexOf(
    'creditActivityRewardOnClient(client',
  );
  const activationIndex = taskVerification.indexOf(
    'activateOnVerifiedActivity(client, row.user_id)',
  );
