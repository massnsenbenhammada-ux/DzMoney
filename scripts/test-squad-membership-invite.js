...
async function phase5CreateUser(walletService, tag, index, balance = 10000) {
  const user = await walletService.createUser({
    telegramUserId: String(920000000 + ((Date.now() % 1000000) * 1000) + index),
    username: `phase5_invite_${tag}_${index}_${Date.now()}`
  });
...
async function phase5Pending(user, maxMembers, idempotencyKey, createdAt = null) {
  const { query } = require('../src/db/pool');
  return query(
    `INSERT INTO squad_membership_purchase_requests
      (user_id,idempotency_key,min_members,max_members,price,status,created_at)
     VALUES($1,$2,$3,$4,$5,'pending',COALESCE($6,NOW())) RETURNING id`,
    [user.id, idempotencyKey, 11, maxMembers, 200, createdAt]
  );
}
...
test('Phase 5: existing-Squad event settles FIFO pending requests only on that Squad', phase5Integration, async () => {
  ...
  try {
    for (let i = 1; i <= 13; i += 1) users.push(await phase5CreateUser(walletService, tag, i));
    ...
    await phase5Pending(users[11], 20, 'fifo-old', '2026-01-01T00:00:00Z');
    await phase5Pending(users[12], 20, 'fifo-new', '2026-01-02T00:00:00Z');
    ...
    const requests = await query(`SELECT user_id,status FROM squad_membership_purchase_requests WHERE user_id=ANY($1::bigint[]) ORDER BY created_at,id`, [[users[11].id, users[12].id]]);
    assert.deepEqual(requests.rows.map(row => [String(row.user_id), row.status]), [[String(users[11].id), 'settled'], [String(users[12].id), 'settled']]);
    assert.equal((await query('SELECT COUNT(*)::int AS count FROM squad_memberships WHERE squad_id=$1 AND user_id=ANY($2::bigint[])', [squads[0], [users[11].id, users[12].id]])).rows[0].count, 2);
    ...
    assert.equal((await query("SELECT COUNT(*)::int AS count FROM ledger_transactions WHERE user_id=ANY($1::bigint[]) AND transaction_type='SQUAD_MEMBERSHIP_PURCHASE'", [[users[11].id, users[12].id]])).rows[0].count, 2);
  } finally {
    process.env.BOT_TOKEN = previousBotToken;
    await phase5Cleanup({ users, squads });
  }
});
...
test('Phase 5: remaining capacity is a hard bound for one settlement pass', phase5Integration, async () => {
  ...
  try {
    ...
    await phase5Pending(users[20], 20, 'bounded-1');
    await phase5Pending(users[21], 20, 'bounded-2');
    await phase5Pending(users[22], 20, 'bounded-3');
