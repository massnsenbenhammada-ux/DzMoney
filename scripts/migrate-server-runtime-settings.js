...
const alreadyMigrated =
  source.includes('const runtimeSettings = require("./runtime-settings-service");') &&
  source.includes('get BUX_PER_TON() { return runtimeSettings.getCachedNumber("bux_per_ton", 10000); }') &&
  source.includes('get COINS_PER_BUX() { return runtimeSettings.getCachedNumber("coins_per_bux", 100); }') &&
  source.includes('runtimeSettings.getWholeNumber("daily_reward_coins", 1000)') &&
  source.includes('runtimeSettings.getWholeNumber("daily_reward_bux", 1)') &&
  source.includes('runtimeSettings.getWholeNumber("daily_reward_cooldown_seconds", 86400)') &&
  source.includes('runtimeSettings.startAutoRefresh(2000)');

if (alreadyMigrated) {
  console.log("Server runtime-settings migration already applied; no changes needed.");
  process.exit(0);
}
