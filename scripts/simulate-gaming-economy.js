...
function choice(weights, random) {
...
function simulateUser(random, config) {
  ...
  return {
    activities: DAYS * ACTIVITY_PER_DAY,
    spins: DAYS * ACTIVITY_PER_DAY + spinAds + extraSpins,
    axes,
    spinAds,
    diggingAds,
    coin,
    ...
    extraSpins,
    extraAxes,
    dzxEquivalent: coin / COIN_PER_DZX + dzx + dzp * DZX_PER_DZP,
  };
}
...
function run(config) {
  ...
  const totals = Object.fromEntries(
    keys.map((key) => [key, users.reduce((sum, user) => sum + user[key], 0)]),
  );
  const averages = Object.fromEntries(
    keys.map((key) => [key, totals[key] / USERS]),
  );
  ...
  const result = {
    ...
    days: DAYS,
    averages,
    bestCase: Math.max(...cost),
    worstCase: Math.min(...cost),
    totalEconomyCostDZXEquivalent: totals.dzxEquivalent,
  };
  console.log(JSON.stringify(result, null, 2));
  if (averages.dzxEquivalent >= 1200)
    throw new Error(
      `Gaming average economic cost ${averages.dzxEquivalent.toFixed(2)} DZX exceeds 1200 DZX guardrail`,
    );
  return result;
}
