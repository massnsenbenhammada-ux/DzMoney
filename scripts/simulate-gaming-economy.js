...
const spinWeights = { coin_1000:440, dzx_1:20, dzx_10:2, dzp_1:4, dzp_10:1, extra_spin:16, none:1500 };
const diggingWeights = { coin_1000:3, dzx_1:1, dzx_10:0, dzp_1:1, dzp_10:0, extra_axe:1, none:10 };

function rng(seed) { let value = seed >>> 0; return () => { value = (value * 1664525 + 1013904223) >>> 0; return value / 0x100000000; }; }
function choice(weights, random) { const entries = Object.entries(weights).filter(([, weight]) => weight > 0); const total = entries.reduce((sum, [, weight]) => sum + weight, 0); let pick = Math.floor(random() * total); for (const [key, weight] of entries) { pick -= weight; if (pick < 0) return key; } return entries[entries.length - 1][0]; }
...
function simulateUser(random) {
  ...
  for (let day = 0; day < DAYS; day += 1) {
    ...
    axes += Math.floor(ACTIVITY_PER_DAY / 10);
