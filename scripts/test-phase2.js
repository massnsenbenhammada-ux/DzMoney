...
async function startVerificationAd(attemptId, idempotencyKey, externalAdId) {
  return startTaskVerificationAd({
    attemptId,
    idempotencyKey,
    externalAdId,
    providerRegistry: adProviderRegistry,
  });
}

async function verifyAd(adEventId, providerPayload) {
  return verifyTaskAdvertisement({
    adEventId,
    providerRegistry: adProviderRegistry,
    providerPayload,
  });
...
async function main() {
  ...
  try {
    ...
    const execution3 = await executeTask({
      taskId: task.id,
      userId,
      idempotencyKey: `phase2-exec-3-${Date.now()}`,
    });
    ...
    await assert.rejects(
      () =>
        finalizeTaskVerification({
          ...
          idempotencyKey: `phase2-invalid-verifier-${Date.now()}`,
          verifyTaskCompletion: async () => ({ verified: true }),
        }),
    ...
    const execution2 = await executeTask({
      taskId: task.id,
      userId,
      idempotencyKey: `phase2-exec-2-${Date.now()}`,
    });
    ...
    const ads = await pool.query(
      `SELECT context, verified, metadata->>'provider_id' AS provider_id FROM activity_ad_events WHERE user_id=$1 ORDER BY id`,
      [userId],
    );
