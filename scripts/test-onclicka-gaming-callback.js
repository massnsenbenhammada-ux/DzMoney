...
async function createEvent(userId, metadata, suffix = '') {
  const result = await query(
    ...
    [
      userId,
      `onclicka-test-${userId}-${suffix || Date.now()}`,
      `onclicka-gaming-${userId}-${suffix || Date.now()}`,
      metadata,
    ],
  );
  return result.rows[0];
...
async function testHappyPathAndDuplicate(app) {
  ...
  try {
    ...
    const event = await query('SELECT metadata FROM activity_ad_events WHERE id=$1', [
      started.adEvent.id,
    ]);
    ...
    assert(
      metadata.ad_bonus && (metadata.ad_bonus.coin === 100 || metadata.ad_bonus.dzx === 1),
    ...
    const directDuplicate = await gamingService.finalizeGamingAdvertisement({
      ...
      verificationMetadata: {
        ...
        spot_id: process.env.ONCLICKA_SPOT_ID,
        telegram_user_id: marker,
...
async function testLateCallback(app) {
  ...
  try {
    ...
    await query('UPDATE activity_ad_events SET verified=TRUE,completed_at=NOW() WHERE id=$1', [
      event.id,
    ]);
    const response = await request(app, `/api/ads/onclicka?USERID=${marker}`);
    assert.strictEqual(response.status, 404);
