...
function testGamingFrontendContract() {
  ...
  const configMarker =
    /<script>\s*window\.__DzMoneyAdProviderConfig\s*=\s*__AD_PROVIDER_CONFIG__\s*;\s*<\/script>/;
  ...
  const configMatch = configMarker.exec(html);
  const configIndex = configMatch?.index ?? -1;
