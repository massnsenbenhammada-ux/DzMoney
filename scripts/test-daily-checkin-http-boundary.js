...
async function run() {
  ...
  require.cache[dailyTasksPath].exports = {
    ...
    getSystemTask: async () => ({
      id: 11,
      config: { dailyPolicy: "rolling_24h" },
    }),
  ...
  require.cache[poolPath].exports = {
    ...
    query: async (text) =>
      ...
      text.includes("status='verification_pending'")
        ? {
            rows: [{ id: 42, executed_at: new Date().toISOString() }],
            rowCount: 1,
          }
        : { rows: [], rowCount: 0 },
  };
  ...
  const {
    createDailyCheckinRouter,
  } = require("../src/http/daily-checkin-routes");
  const app = express();
  app.use(express.json());
  app.use(
    "/api/daily-checkin",
    createDailyCheckinRouter({ providerRegistry: {} }),
  );
  ...
  assert.strictEqual(
    (await request("GET", "/api/daily-checkin/status", null)).status,
    401,
  );
  ...
  assert.strictEqual(
    (
      await request("POST", "/api/daily-checkin/claim", {
  ...
  assert.strictEqual(
    (await request("POST", "/api/daily-checkin/verify", { adEventId: 7 }, auth))
      .status,
    404,
  );
  assert.strictEqual(
