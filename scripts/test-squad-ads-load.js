...
async function main() {
  ...
  try {
      ...
      Array.from({ length: 24 }, (_, index) =>
        (async () => {
          ...
          const result = await startTaskAdvertisement({
            userId,
            taskId,
            idempotencyKey: `squad-load-${marker}-${index}`,
    ...
    assert.equal(
      (
        await pool.query(
          "SELECT COUNT(*)::int AS count FROM activity_ad_events WHERE user_id=$1 AND context='squad'",
          [userId],
        )
      ).rows[0].count,
      24,
    ...
    assert.ok(
      p95 < 2000,
      `Squad advertisement allocation p95 too high: ${p95.toFixed(1)}ms`,
    );
    console.log(
      `Squad Ads load gate: PASS (n=${timings.length}, p95=${p95.toFixed(1)}ms, max=${Math.max(...timings).toFixed(1)}ms)`,
    );
  } finally {
