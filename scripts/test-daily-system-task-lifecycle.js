...
function testSystemTaskIdentifiers() {
  ...
  assert.strictEqual(
    DAILY_SYSTEM_TASKS.SHARE_WITH_FRIENDS,
    'share_with_friends',
  );
...
function testUtcPlusOneCalendarReset() {
  ...
  assert.strictEqual(
    isUtcPlusOneCalendarDayAvailable(previous, sameCalendarDay),
    false,
  );
  assert.strictEqual(
    isUtcPlusOneCalendarDayAvailable(previous, nextCalendarDay),
    true,
  );
}
...
function testCanonicalCheckinMigrationAndUi() {
  const migration = fs.readFileSync(
    path.join(
      ...
      '..',
      'migrations',
      '027_daily_checkin_canonical_task.sql',
    ),
    'utf8',
  );
