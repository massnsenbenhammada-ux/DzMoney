...
function telegramMembershipVerifier({ botToken, expectedTelegramUserId, member }) {
  let calls = 0;
...
async function main() {
  ...
  try {
    ...
    assert.deepStrictEqual(rejected, { duplicate: false, status: 'rejected', rewarded: false });
    assert(rejectedTelegram.getCalls() >= 1, 'Telegram membership verifier must be called');
    assert.deepStrictEqual(await walletBalances(user.id), before, 'Non-member must receive no reward');
    ...
    assert.strictEqual(execution.gate.ad_event_id, null, 'Check for Update must not create an ad event');
    ...
    const results = await Promise.all([
      finalizeTaskVerification({ attemptId: execution.attempt.id, idempotencyKey: concurrentKey, verifyTaskCompletion: acceptedTelegram.verifier }),
      finalizeTaskVerification({ attemptId: execution.attempt.id, idempotencyKey: `${concurrentKey}-concurrent`, verifyTaskCompletion: acceptedTelegram.verifier })
    ]);
    ...
    assert.strictEqual(rewarded.length, 1, 'Concurrent Check for Update claims must create one reward');
    assert.strictEqual(duplicates.length, 1, 'Concurrent Check for Update claim must produce one duplicate result');
    assert.deepStrictEqual(rewarded[0].reward, {
      coin: expected.COIN,
      dzx: expected.DZX,
      dzp: expected.DZP
    });
    assert(acceptedTelegram.getCalls() >= 1, 'Accepted claim must use Telegram membership verifier');
    ...
    assert.strictEqual(transactions.rowCount, 1, 'Check for Update must create exactly one task reward transaction');
    ...
    const adEvents = await pool.query('SELECT id FROM activity_ad_events WHERE user_id=$1', [user.id]);
    assert.strictEqual(adEvents.rowCount, 0, 'Check for Update must not create advertisement events');
    ...
    assert.deepStrictEqual(await walletBalances(user.id), after, 'Retry must not grant a second reward');
