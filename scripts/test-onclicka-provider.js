...
async function testProviderContract() {
  ...
  const result = await provider.verifyCompletion({
    USERID: "12345",
    spot_id: "6134799",
  ...
  const serverResult = await provider.verifyServerCompletion({
    USERID: "12345",
    spot_id: "6134799",
    confirmedByPostback: true,
    reference: "daily-ad-event-1",
  });
...
function testOnclickaIsActiveAtRuntime() {
  for (const context of ONCLICKA_CONTEXTS) {
    assert.strictEqual(
      providerRegistry.listAvailable(context)[0].id,
