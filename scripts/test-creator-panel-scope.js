...
const creatorCss = fs.readFileSync('public/creator-task.css', 'utf8');
...
if (!categoriesBody.includes('setTaskModeTabsVisible(true)')) {
  throw new Error(
    'Tasks landing surface must expose the task/creator mode tabs',
  );
}
if (!categoriesBody.includes('creatorPanel.hidden = true')) {
  throw new Error(
    'Creator panel must remain hidden on the task category landing surface',
  );
}
if (categoriesBody.includes('creatorPanel.hidden = false')) {
  throw new Error(
    'Creator panel must not be forced visible on the task category landing surface',
  );
}
...
if (!categoryBody.includes('creatorPanel.hidden = true')) {
  throw new Error(
    'Creator panel must be hidden inside a specific task category',
  );
}
...
if (/\.creator-panel\{display:block\}/.test(creatorCss)) {
  throw new Error(
    'Creator CSS must not override the hidden attribute with display:block',
  );
}

console.log('CREATOR_PANEL_LANDING_SCOPE: PASS');
console.log('CREATOR_PANEL_CATEGORY_SCOPE: PASS');
console.log('CREATOR_PANEL_HIDDEN_CSS: PASS');
